...
hook OnScriptInit() 
{
    for(new i = 0; i < sizeof Hotdogs; i ++) {
        ...
        pos[0] = Hotdogs[i][hotdog_X] - (1.0 * floatsin(-rz, degrees));
        pos[1] = Hotdogs[i][hotdog_Y] - (1.0 * floatcos(-rz, degrees));
        CreateDynamic3DTextLabel("Presiona {"#PRIMARY_COLOR"}[ H ]{FFFFFF} para comprar comida", 0xFFFFFFFF, pos[0], pos[1], pos[2], 5.0, .testlos = true, .interiorid = Hotdogs[i][hotdog_INTERIOR], .worldid = Hotdogs[i][hotdog_WORLD]);
...
CMD:comida(playerid, params[])
{
    ...
    ShowPlayerDialog(playerid, DIALOG_HOTDOG, DIALOG_STYLE_TABLIST_HEADERS, "{"#PRIMARY_COLOR"}Puesto de comida rápida", dialog_body, "Comprar", "Cerrar");
	return 1;
}
