...
hook OnPlayerConnect(playerid) 
{
	...
	pSpeedoTd[playerid][3] = CreatePlayerTextDraw(playerid, 503.000000, 369.000000, "Gasolina:_0.0_LTS");
...
ShowPlayerSpeedoMeter(playerid)
{
	...
	new td_str[64];
	format(td_str, sizeof td_str, "Veh¢culo:~g~_%s", VEHICLE_INFO[modelid - 400][vehicle_info_NAME]);
	PlayerTextDrawSetString(playerid, pSpeedoTd[playerid][2], td_str);
	
	format(td_str, sizeof td_str, "Gasolina:~g~_%.1f_LTS", GLOBAL_VEHICLES[vehicleid][gb_vehicle_GAS]);
	PlayerTextDrawSetString(playerid, pSpeedoTd[playerid][3], td_str);
	
	PlayerTextDrawSetString(playerid, pSpeedoTd[playerid][4], "Velocidad:~g~_0_Km/h");
...
public UpdatePlayerSpeedo(playerid, vehicleid, Float:maxvel)
{
	...
	new td_str[64];
	format(td_str, 64, "Gasolina:~g~_%.1f_LTS", GLOBAL_VEHICLES[vehicleid][gb_vehicle_GAS]);
	PlayerTextDrawSetString(playerid, pSpeedoTd[playerid][3], td_str);
	
	format(td_str, 64, "Velocidad:~g~_%d_Km/h", floatround(vel));
	PlayerTextDrawSetString(playerid, pSpeedoTd[playerid][4], td_str);

    new start = floatround( floatdiv(vel, floatdiv(maxvel, 33.0)) );
    format(td_str, 64, "~g~IIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIII");
