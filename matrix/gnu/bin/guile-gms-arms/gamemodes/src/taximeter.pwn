...
UpdatePlayerTaxiMeterTextdraws(playerid)
{
	...
	format(td_str, sizeof td_str, "Tarifa:_~g~%d$/Km", TAXI_METER_VEHICLE[vehicleid][veh_taxi_meter_PRICE]);
	PlayerTextDrawSetString(playerid, pTaximeterTd[playerid][2], td_str);
	
	format(td_str, sizeof td_str, "Recorrido:_~g~%.1fKm", TAXI_METER_VEHICLE[vehicleid][veh_taxi_meter_DISTANCE]);
	PlayerTextDrawSetString(playerid, pTaximeterTd[playerid][3], td_str);
	
	format(td_str, sizeof td_str, "A_pagar:_~g~%s$", number_format_thousand(TAXI_METER_VEHICLE[vehicleid][veh_taxi_meter_PAYMENT]));
