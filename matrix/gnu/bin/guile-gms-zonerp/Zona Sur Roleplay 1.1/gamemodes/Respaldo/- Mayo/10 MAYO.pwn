...
//Listo								»	9-Taxista.(Nivel > 2).- Tener conocimiento previo de la ciudad.
					...
					____________________________________________________________________________________________________________________
							...
							Tengamos ganas de seguir con el proyecto Roleplay.
									Copyright(C)- 2012 - 2013 - 2014 Y para las próximas generaciones.
...
new Text:Tit1;
new Text:Tit2;
new Quieretransferir[MAX_PLAYERS];
new Text:Tit3;
new Text:Box1;
new LSPDCoche[16];
new Text:Box2;
...
stock DeleteVehicle(playerid, slot)
{
	...
	format(string, sizeof(string), "Ha borrado de su vehículo en el slot %s.", slotstring);
...
if(dialogid == 1337)
	{
	    if(response)
	    {
	        ...
	        if(GetPVarInt(playerid, "AmountOfCars") >= MAX_OWNABLE_CARS)
				return SendClientMessage(playerid, color_grey, "Usted ya tiene la cantidad máxima de vehículos !"), RemovePlayerFromVehicle(playerid);

			if(GetPlayerMoney(playerid) < price)
			    return SendClientMessage(playerid, color_grey,"No tienes dinero."), RemovePlayerFromVehicle(playerid);
...
CMD:borrarconcesionario(playerid, params[])
{
	...
	if(sscanf(params, "d", id))
	    return SendClientMessage(playerid, color_white, "/destruirconcesionario [ID]");
...
CMD:borrarcoche(playerid, params[])
{
	...
	if(sscanf(params, "d", slot))
	    return SendClientMessage(playerid, color_white, "/Borrar Slot [1/2/3]");
...
CMD:esposar(playerid, params[])
{
		...
		if(IsPlayerConnected(giveplayerid))
		{
			...
			if (PDS(8.0, playerid, giveplayerid))
			{
				...
				else return SendClientMessage(playerid, COLOR_GREY, "El jugador tiene que estar tazeado o con las manos arriba!");
			}
...
PPos(playerid,779.85352, -1708.89819, -16.29786);
return 1;
}
CMD:irint2(playerid,params[]) {
PPos(playerid,793.3901, -1716.8550, -17.9996);
return 1;
}
...
CreateVehicle(556, X,Y,Z, 90.0, 1, 1, -1);
return 1;
}
