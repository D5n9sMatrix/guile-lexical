																																																					/*
											Zona sur roleplay.						     -
					Programador:ScriptNoob- Nazareno                                       -
					Todos los derechos reservados.                                       -
																						 -
					Versión : 1.0
Fecha de inicio de proyecto: 22 de mayo de 2012                                         -      Pronto nuevas actualizaciones.
Fecha de inicio de programación: 24 de agosto del 2013.
    	...
    	Si se ve script en ingles es por que se utilizó un fs en ingles, pero está los créditos!
    	
    	Concesionario base: SlashQ's
...
3-Sistema de banco básico. - Depositar-Sacar-Estado.
...
44- Sistema de agenda. ( 5 NÚMEROS SE PODRÁ GUARDAR). //Se borró
...
51- Sistema de robo y vender.
52- Modificación de daño con diversas armas.
53- Sistema de multas.
54- Sistema de vendas.
55- Sistema de compras de ACCIONES.
56- Sistema de demandas. ( Demandas = Ofertas) .
57- Sistema de SED.
58- Sistema de Apetito.
59- Sistema de Energía.




ADVERTENCIA AL AGREGAR UN VEHÍCULO AL CONCESIONARIO TIENE QUE BORRAR TODOS LOS DEMÁS !
...
LISTA DE TRABAJOS:
//Listo									»	1-Recogedor de basura.
/Listo								»	2-Chofer de bus urbano.
//Listo								»	3-Chofer de bus A larga distancia.
//En progreso								»	4-Barredor de calles.
//Listo.								»	5-Piloto
								»	6-Agricultor.
//Listo								»	7-Agricultor de drogas.
//Listo								»	8-Transportador de mercadería.
//Listo								»	9-Taxista.(Nivel > 2).- Tener conocimiento previo de la ciudad.
					...
					____________________________________________________________________________________________________________________
							...
							Tengamos ganas de seguir con el proyecto Roleplay.
									Copyright(C)- 2012 - 2013 - 2014.



		IDEAS:
//Listo.		Sistema de gps.(Localizar sospechoso).
//Listo		Sistema de Gasolina.
//Listo	Sistema de Negocios.
//Listo		Sistema de concesionarios.
//Listo.	Sistema de compras de productos con canasta- Poner /obtenercanasta , se le aplicará un objeto al jugador , irá a un pick up y pondrá /agarrarp
...
//Listo pero no terminado.	Sistema de inventario.
//Listo		Sistema de renta de vehículos.
		Bares.
//Listo		Sistema de drogas.
//Listo		Sistema monetario de gobierno, se guardara el valor de cada payday 2000 y el gobierno pagar con eso a las facciones.
//Listo.		Sistema de anti abusos admin's, 3 advertencia será removido el admin directamente.
		Sistemas de robo al banco y gasolinera
		Sistema de mercado negro.
//Listo		Sistema de celular
		Sistema de peaje.
//Listo.		Sistema de prestamo.
//Listo		Animaciones.
		Interiores de fac's.
//otra versión				Balas de goma para la policía.
//Listo		Sistema de licencias
//otra versión		Robo a tiendas.
//Listo		Trabajo de chofer de bus URBANO AVANZADO (Tiene que ir a buscar que le toca después termina, después tiene que ir a ver que le toca):
		Exposición de autos en el centro comercial.
		
		-
//Listo		Sistema de Amu nation
//Listo		Sistema de enredar.
//Listo		Sistema de vendar.

			Sistema de bomberos.
//Listo			Vender coche - Vendará coche.
//Listo			Sistema de Hambre.
//Listo			Sistema de liquido.
//Listo			Vender vehículo al usuario.
			Nuevos mappeos.
			
			Sistema de BÁTERIA.
			Sistema de control de coche.
		    ...
		    __________________________________________________________________________________________________________
			    ...
			    ZONA SUR ROLEPLAY GAMEMODE UTILITARIO CREATED BY SCRIPTNOOB - SE PROHIBE LA VENTA DE ÉSTE GAMEMODE.
			__________________________________________________________________________________________________________
...
AntiDeAMX()
{
   ...
   #pragma unused a
	...
	#define MAX_OWNABLE_CARS    2 // Limit of cars a player can own. The higher the more lag
...
new Modelodeusuario[2][MAX_PLAYERS],Preciodeusuario[2][MAX_PLAYERS];
...
new BusUrbanoC[9];
new GarageLSPD[2];
new ModeloC[MAX_PLAYERS],CarID[MAX_PLAYERS];
new ECajero[MAX_PLAYERS];
new EMeca[16];
new ColorP[MAX_PLAYERS],ColorS[MAX_PLAYERS];
new Slotventa[MAX_PLAYERS];
new Text3D:Drogast;
...
Mensaje(playerid,COLOR_BLANCO,"! Te estás quedando sin energía, ve a comer algo nutritivo.");
new Float:Vida;
GetPlayerHealth(playerid,Vida);
SetPlayerHealth(playerid,Vida-5);
}
if(SedPJ[playerid] <= 50) {
Mensaje(playerid,COLOR_BLANCO,"! Tienes mucha sed, ve a tomar algo. De lo contrario perderás energía.");
...
ControlP(playerid,0);
Mensaje(playerid,COLOR_BLANCO,"Espere por favor, se están cargando los objetos..");
}
...
//Sistemas de necesidades - ZSRP - COPYRIGHT..
...
stock SavePlayerVehicleData(playerid)
{
	...
	format(file, sizeof(file), "Archivos de Coches/Coches/Propietarios/%s.vehicle", playername);
	if(INI_Open(file))
	{
		...
		{
	 		format(var, sizeof(var), "Comprado_%d", s);
			...
			format(var, sizeof(var), "ValorC_%d", s);
			INI_WriteInt(var, GetPVarInt(playerid, var));
		}
		INI_Save();
		INI_Close();
...
stock LoadPlayerVehicleData(playerid)
{
	...
	if(fexist(file) && INI_Open(file))
	{
        ...
        Modelodeusuario[0][playerid] = GetPVarInt(playerid,"Model_1");
        Modelodeusuario[1][playerid] = GetPVarInt(playerid,"Model_2");
        Preciodeusuario[0][playerid] = GetPVarInt(playerid,"ValorC_1");
        Preciodeusuario[1][playerid] = GetPVarInt(playerid,"ValorC_2");
		INI_Close();
	}
	for(new s = 1; s <= MAX_OWNABLE_CARS; s++)
	{
		...
		carid = CreateVehicle(model, Floats[1], Floats[2], Floats[3], Floats[4], 1, 1, -1);
...
/*		vehicleid = CreateVehicle(tmp_model, tmp_pos[1], tmp_pos[2], tmp_pos[3], tmp_rot, 1, 1, -1);*/
		...
		format(string, sizeof(string), "%s estacionó su vehículo.", playername);
...
stock ModifyVehicles(playerid)
{
	...
	for(new s = 1; s <= MAX_OWNABLE_CARS; s++)
	{
		...
		if(vehicleid > 0)
		{
  			if(GetPVarInt(playerid,"Model_1") == GetVehicleModel(vehicleid))
	    	{
      		format(var, sizeof(var), "Paintjob_1", s);
			if(GetPVarInt(playerid, var) != -1)
				ChangeVehiclePaintjob(vehicleid, GetPVarInt(playerid, var));

			format(var, sizeof(var), "Component1_1", s);
			if(GetPVarInt(playerid, var) != 0)
				AddVehicleComponent(vehicleid, GetPVarInt(playerid, var));

			format(var, sizeof(var), "Component2_1", s);
			if(GetPVarInt(playerid, var) != 0)
				ChangeVehiclePaintjob(vehicleid, GetPVarInt(playerid, var));

			format(var, sizeof(var), "Component3_1", s);
			if(GetPVarInt(playerid, var) != 0)
				ChangeVehiclePaintjob(vehicleid, GetPVarInt(playerid, var));

			format(var, sizeof(var), "Component4_1", s);
			if(GetPVarInt(playerid, var) != 0)
				AddVehicleComponent(vehicleid, GetPVarInt(playerid, var));

			format(var, sizeof(var), "Component5_1", s);
			if(GetPVarInt(playerid, var) != 0)
				ChangeVehiclePaintjob(vehicleid, GetPVarInt(playerid, var));

			format(var, sizeof(var), "Component6_1", s);
			if(GetPVarInt(playerid, var) != 0)
				ChangeVehiclePaintjob(vehicleid, GetPVarInt(playerid, var));

			format(var, sizeof(var), "Component7_1", s);
			if(GetPVarInt(playerid, var) != 0)
				AddVehicleComponent(vehicleid, GetPVarInt(playerid, var));

			format(var, sizeof(var), "Component8_1", s);
			if(GetPVarInt(playerid, var) != 0)
				ChangeVehiclePaintjob(vehicleid, GetPVarInt(playerid, var));

			format(var, sizeof(var), "Component9_1", s);
			if(GetPVarInt(playerid, var) != 0)
				ChangeVehiclePaintjob(vehicleid, GetPVarInt(playerid, var));

			format(var, sizeof(var), "Component10_1", s);
			if(GetPVarInt(playerid, var) != 0)
				AddVehicleComponent(vehicleid, GetPVarInt(playerid, var));

			format(var, sizeof(var), "Component11_1", s);
			if(GetPVarInt(playerid, var) != 0)
				ChangeVehiclePaintjob(vehicleid, GetPVarInt(playerid, var));

			format(var, sizeof(var), "Component12_1", s);
			if(GetPVarInt(playerid, var) != 0)
				ChangeVehiclePaintjob(vehicleid, GetPVarInt(playerid, var));

			format(var, sizeof(var), "Component13_1", s);
			if(GetPVarInt(playerid, var) != 0)
				AddVehicleComponent(vehicleid, GetPVarInt(playerid, var));

			format(var, sizeof(var), "Component14_1", s);
			if(GetPVarInt(playerid, var) != 0)
				ChangeVehiclePaintjob(vehicleid, GetPVarInt(playerid, var));

			format(var, sizeof(var), "Component15_1", s);
			if(GetPVarInt(playerid, var) != 0)
				ChangeVehiclePaintjob(vehicleid, GetPVarInt(playerid, var));

			format(var, sizeof(var), "Component16_1", s);
			if(GetPVarInt(playerid, var) != 0)
				AddVehicleComponent(vehicleid, GetPVarInt(playerid, var));

			format(var, sizeof(var), "Component17_1", s);
			if(GetPVarInt(playerid, var) != 0)
				ChangeVehiclePaintjob(vehicleid, GetPVarInt(playerid, var));

			format(var, sizeof(var), "Color1_1", s);
			colors[1] = GetPVarInt(playerid, var);

			format(var, sizeof(var), "Color2_1", s);
			colors[2] = GetPVarInt(playerid, var);

			ChangeVehicleColor(vehicleid, colors[1], colors[2]);
			}
			else if(GetPVarInt(playerid,"Model_2") == GetVehicleModel(playerid)) {
			      		format(var, sizeof(var), "Paintjob_2", s);
			if(GetPVarInt(playerid, var) != -1)
				ChangeVehiclePaintjob(vehicleid, GetPVarInt(playerid, var));

			format(var, sizeof(var), "Component1_2", s);
			if(GetPVarInt(playerid, var) != 0)
				AddVehicleComponent(vehicleid, GetPVarInt(playerid, var));

			format(var, sizeof(var), "Component2_2", s);
			if(GetPVarInt(playerid, var) != 0)
				ChangeVehiclePaintjob(vehicleid, GetPVarInt(playerid, var));

			format(var, sizeof(var), "Component3_2", s);
			if(GetPVarInt(playerid, var) != 0)
				ChangeVehiclePaintjob(vehicleid, GetPVarInt(playerid, var));

			format(var, sizeof(var), "Component4_2", s);
			if(GetPVarInt(playerid, var) != 0)
				AddVehicleComponent(vehicleid, GetPVarInt(playerid, var));

			format(var, sizeof(var), "Component5_2", s);
			if(GetPVarInt(playerid, var) != 0)
				ChangeVehiclePaintjob(vehicleid, GetPVarInt(playerid, var));

			format(var, sizeof(var), "Component6_2", s);
			if(GetPVarInt(playerid, var) != 0)
				ChangeVehiclePaintjob(vehicleid, GetPVarInt(playerid, var));

			format(var, sizeof(var), "Component7_2", s);
			if(GetPVarInt(playerid, var) != 0)
				AddVehicleComponent(vehicleid, GetPVarInt(playerid, var));

			format(var, sizeof(var), "Component8_2", s);
			if(GetPVarInt(playerid, var) != 0)
				ChangeVehiclePaintjob(vehicleid, GetPVarInt(playerid, var));

			format(var, sizeof(var), "Component9_2", s);
			if(GetPVarInt(playerid, var) != 0)
				ChangeVehiclePaintjob(vehicleid, GetPVarInt(playerid, var));

			format(var, sizeof(var), "Component10_2", s);
			if(GetPVarInt(playerid, var) != 0)
				AddVehicleComponent(vehicleid, GetPVarInt(playerid, var));

			format(var, sizeof(var), "Component11_2", s);
			if(GetPVarInt(playerid, var) != 0)
				ChangeVehiclePaintjob(vehicleid, GetPVarInt(playerid, var));

			format(var, sizeof(var), "Component12_2", s);
			if(GetPVarInt(playerid, var) != 0)
				ChangeVehiclePaintjob(vehicleid, GetPVarInt(playerid, var));

			format(var, sizeof(var), "Component13_2", s);
			if(GetPVarInt(playerid, var) != 0)
				AddVehicleComponent(vehicleid, GetPVarInt(playerid, var));

			format(var, sizeof(var), "Component14_2", s);
			if(GetPVarInt(playerid, var) != 0)
				ChangeVehiclePaintjob(vehicleid, GetPVarInt(playerid, var));

			format(var, sizeof(var), "Component15_2", s);
			if(GetPVarInt(playerid, var) != 0)
				ChangeVehiclePaintjob(vehicleid, GetPVarInt(playerid, var));

			format(var, sizeof(var), "Component16_2", s);
			if(GetPVarInt(playerid, var) != 0)
				AddVehicleComponent(vehicleid, GetPVarInt(playerid, var));

			format(var, sizeof(var), "Component17_2", s);
			if(GetPVarInt(playerid, var) != 0)
				ChangeVehiclePaintjob(vehicleid, GetPVarInt(playerid, var));

			format(var, sizeof(var), "Color1_2", s);
			colors[1] = GetPVarInt(playerid, var);

			format(var, sizeof(var), "Color2_2", s);
			colors[2] = GetPVarInt(playerid, var);
...
stock DeleteVehicle(playerid, slot)
{
	...
	format(var, sizeof(var), "ValorC_%d", slot);
	SetPVarInt(playerid, var, 0);
	format(var, sizeof(var), "Comprado_%d", slot);
	SetPVarInt(playerid, var, 0);
	...
	switch(slot)
	{
	    case 1: slotstring = "uno";
	    case 2: slotstring = "dos";
	}
	format(string, sizeof(string), "Has vendido tu vehículo en el slot %s.", slotstring);
...
SetTimer("RegenerarpuertaB1",280000,0);
return 1;
}
...
ClearAnimations(playerid);
SetTimer("RegenerarpuertaB2",280000,0);
return 1;
}
...
else if(PlayerToPoint(playerid, 2.0,1367.7000000,-1279.8000000,14.2000000)) { //Entrada al ammunation
PPos(playerid,285.4000000,-41.2000000,1002.2000000);
abrirpuerta(playerid);
...
else if(IsPlayerInRangeOfPoint(playerid,2.0,444.646911,508.239044,1001.419494)) {
...
else if(IsPlayerInRangeOfPoint(playerid,2.0,2527.654052,-1679.388305,1015.498596)) {
for(new i = 0; i < MAX_HOUSES; i++)
{
LoadPos(playerid);
cargarjugador(playerid);
...
else if(IsPlayerInRangeOfPoint(playerid,2.0,2807.619873,-1171.899902,1025.570312)) {
for(new i = 0; i < MAX_HOUSES; i++)
{
LoadPos(playerid);
cargarjugador(playerid);
...
else if(IsPlayerInRangeOfPoint(playerid,2.0,1527.229980,-11.574499,1002.097106)) {
for(new i = 0; i < MAX_HOUSES; i++)
{
LoadPos(playerid);
cargarjugador(playerid);
...
else if(IsPlayerInRangeOfPoint(playerid,2.0,2496.049804,-1695.238159,1014.742187)) {
for(new i = 0; i < MAX_HOUSES; i++)
{
LoadPos(playerid);
cargarjugador(playerid);
...
else if(PlayerToPoint(playerid, 5.0,285.4000000,-41.2000000,1002.2000000)) { //Ammu nation
PPos(playerid,1367.7000000,-1279.8000000,14.2000000);
SetPlayerInterior(playerid,0);
cerrolapuerta(playerid);
cargarjugador(playerid);
}
...
stock CreateDealershipVehicle(creator, model, price, Float:carx, Float:cary, Float:carz, Float:carangle)
{
	...
	vehicleid = CreateVehicle(model, carx, cary, carz, carangle, 1, 1, -1);
	...
	format(string, sizeof(string), "{00B3FF}[VEHÍCULO EN VENTA]\n{FFFFFF}Modelo:{00B3FF} %s \n{FFFFFF}Precio:{00DA00} $%d", GetVehicleName(vehicleid), price);
...
stock LoadHouses()
{
	...
	for(new i = 0; i < MAX_HOUSES; i++)
	{
		...
		if(dini_Exists(file))
		{
			...
			PropiedadInfo[i][IntID] = dini_Int(file,"IntT");
			PropiedadInfo[i][CCerrada] = dini_Int(file,"CCerrada");
			PropiedadInfo[i][PropiedadID] = dini_Int(file,"PropiedadID");
			...
			{
				case 0:{format(labelstring,sizeof(labelstring),"Propiedad:\n{0084FF}Nivel:{FFFFFF} %i {0084FF}Precio: {51FF00}$ {FFFFFF}%i\n[ID: %d]"/*,PropiedadInfo[i][CaNombre]*/,PropiedadInfo[i][Nivel],PropiedadInfo[i][Precio],PropiedadInfo[i][PropiedadID]);}
				case 1:{format(labelstring,sizeof(labelstring),"Propiedad:\n{0084FF}Estado: {FFFFFF}No disponible -Vendida-\n {0084FF}Nivel: {FFFFFF}%i\n{0084FF}Precio:{51FF00}${FFFFFF}%i\n{0084FF}Dueño: {FFFFFF}%s\n[ID: %d]"/*,PropiedadInfo[i][CaNombre]*/,PropiedadInfo[i][Nivel],PropiedadInfo[i][Precio],PropiedadInfo[i][Npropietario],PropiedadInfo[i][PropiedadID]);}
			}
...
new vehicleid = GetPlayerVehicleID(playerid);
Gasolina[vehicleid] += 3; CargoGasolina[playerid] += 2;
if(CargarGas[playerid] >= CargoGasolina[playerid]) {
...
if(IsPlayerConnected(i))
{
new String[200],String6[150];
PI[i][SubNivel]++;
Mensaje(i,COLOR_BLANCO, "{FFFFFF}»{00B7FF}IMPUESTOS{FFFFFF}«");
Mensaje(i,COLOR_BLANCO,	"");
Mensaje(i,COLOR_BLANCO, "{CE0000}-{FFFFFF} Asignación:{00D20A} 300$");
...
if(PI[i][TNegocio] == 1) {
if(PI[i][InvirtioN] >= 1) {
format(String,sizeof(String),"{CE0000}-{FFFFFF} Invertiste:{00D20A} $ %d {CE0000}-{FFFFFF}Ganancia:{00D20A} 500$",PI[i][InvirtioN]);
SendClientMessage(i,COLOR_BLANCO,String);
...
new String2[200];
format(String2,sizeof(String2),"El banco te descontó {34CE00}500${FFFFFF} por el prestamo , todavía resta:{34CE00} $%d",PI[i][PrestamoP]-5000);
...
new String2[200];
format(String2,sizeof(String2),"El banco te descontó  {34CE00}1000${FFFFFF} por el prestamo , todavía resta:{34CE00}%d",PI[i][PrestamoP]-15000);
Mensaje(i,COLOR_BLANCO,String2);
...
new String2[200];
format(String2,sizeof(String2),"El banco te descontó {34CE00}2000${FFFFFF} por el prestamo , todavía resta:{34CE00}%d",PI[i][PrestamoP]-20000);
Mensaje(i,COLOR_BLANCO,String2);
...
//CELULAR
	...
	TextDrawBackgroundColor(BoxVelo1, 255);
	TextDrawFont(BoxVelo1, 1);
	TextDrawLetterSize(BoxVelo1, 0.500000, 3.000000);
	TextDrawColor(BoxVelo1, -1);
	TextDrawSetOutline(BoxVelo1, 0);
	TextDrawSetProportional(BoxVelo1, 1);
	TextDrawSetShadow(BoxVelo1, 1);
...
/*
	...
	MRP = TextDrawCreate(54.000000, 432.000000, "~w~ROLEPLAY");
...
/*
	...
	Tit3 = TextDrawCreate(254.000000, 327.000000, "~w~Roleplay");
...
*/
	...
	CreatePickup(1318, 2, 1481.52,-1748.4,15.3,0);//Ayuntamiento EXT AL INT
    ...
    CT("Ayuntamiento\n Ingreso{00B3FF} /Entrar",COLOR_BLANCO,1481.52,-1748.4,15.3,10.0,0,0);
	...
	/*CT("hola",COLOR_BLANCO,2341.16,-1927.94,13.57,10.0,0,0);*/
	...
	EMeca[0] =
	CreateDynamicObject(1698,1215.5000000,-1630.6000000,12.7000000,0.0000000,0.0000000,90.0000000); //1 abajo
...
//CHOFER BUS LARGA DISTANCIA
	...
	CreateObject(1215,1729.6999500,-1742.3000500,13.1000000,0.0000000,0.0000000,0.0000000); //object(bollardlight) (35)
		...
		//Interior banco
	CreateObject(18030, 779.85352, -1708.89819, -16.29786,   0.00000, 0.00000, 0.00000);
	CreateObject(2165, 758.82617, -1719.97302, -18.66904,   0.00000, 0.00000, 0.00000);
	CreateObject(2165, 763.28094, -1719.87170, -18.66904,   0.00000, 0.00000, 0.00000);
	CreateObject(2165, 767.81317, -1719.65662, -18.66904,   0.00000, 0.00000, 0.00000);
	CreateObject(3462, 773.64362, -1721.60413, -17.42600,   0.00000, 0.00000, -90.00000);
	CreateObject(3462, 776.13910, -1721.58301, -17.42600,   0.00000, 0.00000, -90.00000);
	CreateObject(3657, 762.02393, -1695.56506, -18.18110,   0.00000, 0.00000, 0.00000);
	CreateObject(2942, 768.92883, -1695.29529, -18.11839,   0.00000, 0.00000, 0.00000);
	CreateObject(2942, 770.45544, -1695.34338, -18.11839,   0.00000, 0.00000, 0.00000);
	CreateObject(2942, 767.50952, -1695.29968, -18.11839,   0.00000, 0.00000, 0.00000);
	CreateObject(2942, 766.16089, -1695.30127, -18.11839,   0.00000, 0.00000, 0.00000);
	CreateObject(2942, 772.06506, -1695.33264, -18.11839,   0.00000, 0.00000, 0.00000);
	CreateObject(3657, 758.55157, -1697.57410, -18.18110,   0.00000, 0.00000, 90.00000);
	CreateObject(3657, 758.49719, -1701.58008, -18.18110,   0.00000, 0.00000, 90.00000);
	CreateObject(3657, 758.52576, -1706.68774, -18.18110,   0.00000, 0.00000, 90.00000);
	CreateObject(3657, 764.92188, -1697.87524, -18.18110,   0.00000, 0.00000, -90.00000);
	CreateObject(3657, 764.94635, -1701.67041, -18.18113,   0.00000, 0.00000, -90.00000);
	CreateDynamicObject(8648,1484.7000000,-1836.0000000,13.4000000,0.0000000,0.0000000,270.0000000); //object(shbbyhswall04_lvs) (6)
	CreateDynamicObject(8648,1513.9000200,-1836.0000000,13.4000000,0.0000000,0.0000000,270.0000000); //object(shbbyhswall04_lvs) (7)
	CreateDynamicObject(8648,1543.1999500,-1836.0000000,13.4000000,0.0000000,0.0000000,270.0000000); //object(shbbyhswall04_lvs) (8)
	CreateDynamicObject(8648,1432.3000000,-1836.0000000,13.4000000,0.0000000,0.0000000,270.0000000); //object(shbbyhswall04_lvs) (9)
	CreateDynamicObject(8648,1558.2000000,-1821.3000000,13.4000000,0.0000000,0.0000000,180.0000000); //object(shbbyhswall04_lvs) (10)
	CreateDynamicObject(8648,1558.3000000,-1783.0000000,13.4000000,0.0000000,0.0000000,0.0000000); //object(shbbyhswall04_lvs) (12)
	CreateDynamicObject(1251,1394.1000000,-1802.8000000,12.4000000,0.0000000,0.0000000,179.9950000); //object(smashbar) (7)
	CreateDynamicObject(982,1545.3000000,-1836.0000000,14.2000000,0.0000000,0.0000000,270.0000000); //object(fenceshit) (2)
	CreateDynamicObject(982,1494.1000000,-1836.0000000,14.2000000,0.0000000,0.0000000,270.0000000); //object(fenceshit) (3)
	CreateDynamicObject(983,1558.5000000,-1772.9000000,14.2000000,0.0000000,0.0000000,0.0000000); //object(fenceshit3) (1)
	CreateDynamicObject(982,1558.1000000,-1819.9000000,14.2000000,0.0000000,0.0000000,0.0000000); //object(fenceshit) (4)
	CreateDynamicObject(982,1519.6999500,-1836.0000000,14.2000000,0.0000000,0.0000000,270.0000000); //object(fenceshit) (5)
	CreateDynamicObject(982,1494.1000000,-1836.0000000,14.2000000,0.0000000,0.0000000,270.0000000); //object(fenceshit) (6)
	CreateDynamicObject(982,1468.5000000,-1836.0000000,14.2000000,0.0000000,0.0000000,270.0000000); //object(fenceshit) (7)
	CreateDynamicObject(982,1442.9000000,-1836.0000000,14.2000000,0.0000000,0.0000000,270.0000000); //object(fenceshit) (8)
	CreateDynamicObject(982,1419.0000000,-1835.9000000,14.2000000,0.0000000,0.0000000,270.0000000); //object(fenceshit) (9)
	CreateDynamicObject(983,1404.4000000,-1835.9000000,14.2000000,0.0000000,0.0000000,90.0000000); //object(fenceshit3) (2)
	CreateDynamicObject(982,1401.2000000,-1783.5000000,14.3000000,0.0000000,0.0000000,0.0000000); //object(fenceshit) (10)
	CreateDynamicObject(983,1401.3000000,-1832.6000000,14.2000000,0.0000000,0.0000000,0.0000000); //object(fenceshit3) (3)
	CreateDynamicObject(982,1401.2000000,-1783.5000000,13.0000000,0.0000000,0.0000000,0.0000000); //object(fenceshit) (11)
	CreateDynamicObject(966,1401.0000000,-1799.3000000,12.5000000,0.0000000,0.0000000,90.0000000); //object(bar_gatebar01) (1)
	CreateDynamicObject(968,1558.3000000,-1804.3000000,13.5000000,0.0000000,14.9960000,92.0000000); //object(barrierturn) (1)
	CreateDynamicObject(982,1558.5000000,-1784.1000000,14.2000000,0.0000000,0.0000000,0.0000000); //object(fenceshit) (12)
	CreateDynamicObject(10183,1404.6000000,-1774.2000000,12.6000000,0.0000000,0.0000000,136.0000000); //object(ferspaces) (1)
	CreateDynamicObject(10183,1517.4000000,-1796.9000000,12.6000000,0.0000000,0.0000000,136.0000000); //object(ferspaces) (2)
	CreateDynamicObject(10183,1554.7000000,-1778.1000000,12.6000000,0.0000000,0.0000000,316.0000000); //object(ferspaces) (3)
	CreateDynamicObject(10183,1524.1000000,-1832.6000000,12.6000000,0.0000000,0.0000000,226.0000000); //object(ferspaces) (4)
	CreateDynamicObject(10183,1470.1000000,-1797.3000000,12.6000000,0.0000000,0.0000000,136.0000000); //object(ferspaces) (5)
	CreateDynamicObject(10183,1482.8000000,-1796.9000000,12.6000000,0.0000000,0.0000000,316.0000000); //object(ferspaces) (6)
	CreateDynamicObject(10183,1482.8000000,-1796.9000000,12.6000000,0.0000000,0.0000000,316.0000000); //object(ferspaces) (7)
	CreateDynamicObject(10183,1482.8000000,-1796.9000000,12.6000000,0.0000000,0.0000000,316.0000000); //object(ferspaces) (8)
	CreateDynamicObject(10183,1482.8000000,-1796.9000000,12.6000000,0.0000000,0.0000000,316.0000000); //object(ferspaces) (9)
	CreateDynamicObject(10183,1482.8000000,-1796.9000000,12.6000000,0.0000000,0.0000000,316.0000000); //object(ferspaces) (10)
	CreateDynamicObject(10183,1482.8000000,-1796.9000000,12.6000000,0.0000000,0.0000000,316.0000000); //object(ferspaces) (11)
	CreateDynamicObject(10183,1423.4000000,-1832.5000000,12.6000000,0.0000000,0.0000000,226.0000000); //object(ferspaces) (12)
	CreateDynamicObject(10183,1435.4000000,-1795.9000000,12.6000000,0.0000000,0.0000000,316.0000000); //object(ferspaces) (13)
	CreateDynamicObject(10183,1475.5000000,-1833.0000000,12.6000000,0.0000000,0.0000000,226.0000000); //object(ferspaces) (14)
	CreateDynamicObject(3578,1438.7000000,-1732.1000000,11.7000000,0.0000000,0.0000000,270.0000000); //object(dockbarr1_la) (1)
	CreateDynamicObject(3578,1450.3000000,-1732.1000000,11.7000000,0.0000000,0.0000000,270.0000000); //object(dockbarr1_la) (2)
	CreateDynamicObject(3578,1448.5000000,-1732.3000000,11.7000000,0.0000000,0.0000000,270.0000000); //object(dockbarr1_la) (3)
	CreateDynamicObject(3578,1463.3000000,-1732.3000000,11.7000000,0.0000000,0.0000000,270.0000000); //object(dockbarr1_la) (4)
	CreateDynamicObject(3578,1474.1000000,-1732.2000000,11.7000000,0.0000000,0.0000000,270.0000000); //object(dockbarr1_la) (5)
	CreateDynamicObject(3578,1475.6000000,-1732.4000000,11.7000000,0.0000000,0.0000000,270.0000000); //object(dockbarr1_la) (6)
	CreateDynamicObject(3578,1487.1000000,-1732.3000000,11.7000000,0.0000000,0.0000000,270.0000000); //object(dockbarr1_la) (7)
	CreateDynamicObject(3578,1499.1000000,-1732.1000000,11.7000000,0.0000000,0.0000000,270.0000000); //object(dockbarr1_la) (8)
	CreateDynamicObject(3578,1500.6000000,-1732.4000000,11.7000000,0.0000000,0.0000000,270.0000000); //object(dockbarr1_la) (9)
	CreateDynamicObject(7191,1825.8000000,-1112.3000000,25.1000000,0.0000000,0.0000000,178.0000000); //object(vegasnnewfence2b) (8)
	CreateDynamicObject(7191,1825.5000000,-1126.7000000,25.1000000,0.0000000,0.0000000,177.9950000); //object(vegasnnewfence2b) (9)
	CreateDynamicObject(7191,1802.9000000,-1148.9000000,25.1000000,0.0000000,0.0000000,89.9950000); //object(vegasnnewfence2b) (10)
	CreateDynamicObject(8649,1692.3000000,-1136.2000000,23.7000000,0.0000000,0.0000000,2.0000000); //object(shbbyhswall05_lvs) (2)
	CreateDynamicObject(8649,1760.3000000,-1149.3000000,23.7000000,0.0000000,0.0000000,270.0000000); //object(shbbyhswall05_lvs) (5)
	CreateDynamicObject(8649,1715.2000000,-1149.5000000,23.7000000,0.0000000,0.0000000,270.0000000); //object(shbbyhswall05_lvs) (6)
	CreateDynamicObject(983,1736.4000000,-1146.0000000,23.7000000,0.0000000,0.0000000,2.0000000); //object(fenceshit3) (1)
	CreateDynamicObject(983,1712.5000000,-1136.4000000,23.8000000,0.0000000,0.0000000,360.0000000); //object(fenceshit3) (2)
	CreateDynamicObject(983,1727.6000000,-1136.6000000,23.8000000,0.0000000,0.0000000,2.0000000); //object(fenceshit3) (3)
	CreateDynamicObject(983,1722.5000000,-1136.5000000,23.8000000,0.0000000,0.0000000,359.9950000); //object(fenceshit3) (4)
	CreateDynamicObject(983,1717.5000000,-1136.5000000,23.8000000,0.0000000,0.0000000,359.9950000); //object(fenceshit3) (5)
	CreateDynamicObject(983,1707.4000000,-1136.7000000,23.8000000,0.0000000,0.0000000,359.9950000); //object(fenceshit3) (6)
	CreateDynamicObject(983,1697.4000000,-1136.6000000,23.8000000,0.0000000,0.0000000,1.9950000); //object(fenceshit3) (7)
	CreateDynamicObject(983,1702.4000000,-1136.6000000,23.8000000,0.0000000,0.0000000,359.9950000); //object(fenceshit3) (8)
	CreateDynamicObject(982,1692.3000000,-1138.0000000,24.5000000,0.0000000,0.0000000,182.0000000); //object(fenceshit) (2)
	CreateDynamicObject(982,1713.5000000,-1149.3000000,24.5000000,0.0000000,0.0000000,270.0000000); //object(fenceshit) (3)
	CreateDynamicObject(984,1752.4000000,-1149.2000000,24.5000000,0.0000000,0.0000000,270.0000000); //object(fenceshit2) (1)
	CreateDynamicObject(8646,1728.1000000,-1149.4000000,23.8000000,0.0000000,0.0000000,270.0000000); //object(shbbyhswall02_lvs) (1)
	CreateDynamicObject(982,1729.8000000,-1149.2000000,24.5000000,0.0000000,0.0000000,270.0000000); //object(fenceshit) (7)
	CreateDynamicObject(10401,1814.1000000,-1145.7000000,25.0000000,0.0000000,0.0000000,43.9950000); //object(hc_shed02_sfs) (1)
	CreateDynamicObject(9241,1811.6000000,-1104.9000000,24.7000000,0.0000000,0.0000000,357.9950000); //object(copbits_sfn) (2)
	CreateDynamicObject(3660,1755.2000000,-1150.2000000,24.3000000,0.0000000,0.0000000,358.0000000); //object(lasairfbed_las) (1)
000,22.8000000,0.0000000,175.9960000,289.9900000); //object(rockbrkq) (2)
	CreateDynamicObject(905,1735.4000000,-1102.5000000,22.8000000,0.0000000,175.9960000,289.9900000); //object(rockbrkq) (2)
	CreateDynamicObject(905,1738.9000000,-1106.1000000,22.8000000,0.0000000,175.9960000,289.9900000); //object(rockbrkq) (2)
	CreateDynamicObject(905,1738.9000000,-1106.1000000,22.8000000,0.0000000,175.9960000,289.9900000); //object(rockbrkq) (2)
	CreateDynamicObject(7894,1737.8000000,-1109.2000000,23.6000000,0.0000000,0.0000000,359.9860000); //object(vegascrashbar05) (7)
	CreateDynamicObject(1237,1782.8000000,-1117.4000000,23.1000000,0.0000000,0.0000000,0.0000000); //object(strtbarrier01) (3)
	CreateDynamicObject(978,1710.6000000,-1113.1000000,23.9000000,0.0000000,0.0000000,193.9930000); //object(sub_roadright) (20)
	CreateDynamicObject(978,1705.2000000,-1109.1000000,23.9000000,0.0000000,0.0000000,193.9910000); //object(sub_roadright) (20)
	CreateDynamicObject(7894,1696.7000000,-1114.1000000,23.6000000,0.0000000,0.0000000,239.9970000); //object(vegascrashbar05) (2)
	CreateDynamicObject(7894,1700.8000000,-1121.9000000,23.6000000,0.0000000,0.0000000,351.9960000); //object(vegascrashbar05) (2)
	CreateDynamicObject(1350,1769.4000000,-1104.1000000,23.1000000,0.0000000,0.0000000,86.0000000); //object(cj_traffic_light4) (1)
	CreateDynamicObject(3578,1769.4000000,-1109.1000000,22.5000000,0.0000000,0.0000000,271.9950000); //object(dockbarr1_la) (2)
	CreateDynamicObject(3578,1769.7000000,-1118.8000000,22.5000000,0.0000000,0.0000000,271.9940000); //object(dockbarr1_la) (2)
	CreateDynamicObject(978,1771.5000000,-1122.9000000,23.9000000,0.0000000,0.0000000,357.9950000); //object(sub_roadright) (6)
	CreateDynamicObject(978,1764.4000000,-1109.8000000,23.9000000,0.0000000,0.0000000,179.9950000); //object(sub_roadright) (16)
	CreateDynamicObject(1238,1752.1000000,-1103.5000000,23.4000000,0.0000000,0.0000000,0.0000000); //object(trafficcone) (32)
	CreateDynamicObject(1238,1756.7000000,-1103.5000000,23.4000000,0.0000000,0.0000000,0.0000000); //object(trafficcone) (32)
	CreateDynamicObject(1238,1755.1000000,-1103.5000000,23.4000000,0.0000000,0.0000000,0.0000000); //object(trafficcone) (32)
	CreateDynamicObject(978,1764.1000000,-1103.6000000,23.9000000,0.0000000,0.0000000,179.9950000); //object(sub_roadright) (16)
	CreateDynamicObject(1238,1758.2000000,-1103.6000000,23.4000000,0.0000000,0.0000000,0.0000000); //object(trafficcone) (32)
	CreateDynamicObject(3460,1742.7000000,-1101.8000000,27.3000000,0.0000000,0.0000000,126.0000000); //object(vegaslampost) (1)
	CreateDynamicObject(6462,1727.7000000,-1164.2000000,24.7000000,0.0000000,0.0000000,0.0000000); //object(pier04a_law2) (1)
	CreateDynamicObject(1215,1769.4000000,-1109.9000000,23.8000000,0.0000000,0.0000000,0.0000000); //object(bollardlight) (1)
	CreateDynamicObject(1215,1769.3000000,-1104.3000000,23.9000000,0.0000000,0.0000000,0.0000000); //object(bollardlight) (2)
	CreateDynamicObject(1251,1824.2000000,-1147.0000000,23.0000000,0.0000000,0.0000000,0.0000000); //object(smashbar) (1)
	CreateDynamicObject(1251,1814.1000000,-1146.8000000,23.1000000,0.0000000,0.0000000,0.0000000); //object(smashbar) (2)
	CreateDynamicObject(1251,1819.0000000,-1147.0000000,23.0000000,0.0000000,0.0000000,0.0000000); //object(smashbar) (3)
	CreateDynamicObject(1251,1809.1000000,-1146.7000000,23.0000000,0.0000000,0.0000000,0.0000000); //object(smashbar) (4)
	CreateDynamicObject(1251,1804.1000000,-1146.6000000,23.0000000,0.0000000,0.0000000,0.0000000); //object(smashbar) (5)
	CreateDynamicObject(978,1817.9000000,-1128.1000000,23.9000000,0.0000000,0.0000000,359.9910000); //object(sub_roadright) (5)
13.2000000,0.0000000,0.0000000,0.0000000); //object(veg_pflowers2wee) (5)
	CreateDynamicObject(870,1547.6999500,-1661.6999500,13.2000000,0.0000000,0.0000000,0.0000000); //object(veg_pflowers2wee) (6)
	CreateDynamicObject(870,1547.4000200,-1657.9000200,13.2000000,0.0000000,0.0000000,0.0000000); //object(veg_pflowers2wee) (7)
	CreateDynamicObject(870,1547.8000500,-1686.0000000,13.2000000,0.0000000,0.0000000,0.0000000); //object(veg_pflowers2wee) (8)
	CreateDynamicObject(870,1547.4000200,-1688.4000200,13.2000000,0.0000000,0.0000000,0.0000000); //object(veg_pflowers2wee) (9)
	CreateDynamicObject(870,1547.4000200,-1691.0999800,13.2000000,0.0000000,0.0000000,0.0000000); //object(veg_pflowers2wee) (10)
	CreateDynamicObject(870,1547.3000500,-1693.8000500,13.2000000,0.0000000,0.0000000,0.0000000); //object(veg_pflowers2wee) (11)
	CreateDynamicObject(870,1545.1999500,-1693.5999800,13.2000000,0.0000000,0.0000000,0.0000000); //object(veg_pflowers2wee) (12)
	CreateDynamicObject(870,1545.3000500,-1690.8000500,13.2000000,0.0000000,0.0000000,0.0000000); //object(veg_pflowers2wee) (13)
	CreateDynamicObject(870,1545.1999500,-1688.3000500,13.2000000,0.0000000,0.0000000,0.0000000); //object(veg_pflowers2wee) (14)
	CreateDynamicObject(870,1545.3000500,-1686.4000200,13.2000000,0.0000000,0.0000000,0.0000000); //object(veg_pflowers2wee) (15)
	CreateDynamicObject(1361,1554.5000000,-1677.9000200,15.9000000,0.0000000,0.0000000,0.0000000); //object(cj_bush_prop2) (1)
	CreateDynamicObject(1361,1554.5999800,-1673.3000500,15.9000000,0.0000000,0.0000000,0.0000000); //object(cj_bush_prop2) (2)
	CreateDynamicObject(3578,1529.5000000,-1602.0999800,11.6100000,0.0000000,0.0000000,0.0000000); //object(dockbarr1_la) (1)
	CreateDynamicObject(3578,1529.5000000,-1639.1992200,11.6600000,0.0000000,0.0000000,0.0000000); //object(dockbarr1_la) (2)
	CreateDynamicObject(3578,1529.5999800,-1617.1999500,11.6600000,0.0000000,0.0000000,0.0000000); //object(dockbarr1_la) (3)
	CreateDynamicObject(3578,1529.5000000,-1722.6999500,11.6100000,0.0000000,0.0000000,0.0000000); //object(dockbarr1_la) (4)
	CreateDynamicObject(3578,1554.6999500,-1617.5999800,11.9000000,0.0000000,0.0000000,0.0000000); //object(dockbarr1_la) (5)
	CreateDynamicObject(3578,1565.0000000,-1617.5999800,11.9000000,0.0000000,0.0000000,0.0000000); //object(dockbarr1_la) (6)
	CreateDynamicObject(3578,1570.0999800,-1617.5999800,11.9000000,0.0000000,0.0000000,0.0000000); //object(dockbarr1_la) (7)
	CreateDynamicObject(982,1562.4000200,-1617.5999800,13.4000000,0.0000000,0.0000000,90.0000000); //object(fenceshit) (1)
	CreateDynamicObject(1251,1575.1992200,-1605.6999500,12.4000000,0.0000000,0.0000000,0.0000000); //object(smashbar) (1)
	CreateDynamicObject(1251,1575.1992200,-1613.8994100,12.4000000,0.0000000,0.0000000,0.0000000); //object(smashbar) (2)
	CreateDynamicObject(1251,1575.1992200,-1609.7998000,12.4000000,0.0000000,0.0000000,0.0000000); //object(smashbar) (3)
	CreateDynamicObject(983,1544.8000500,-1620.8000500,13.2000000,0.0000000,0.0000000,0.0000000); //object(fenceshit3) (1)
	CreateDynamicObject(983,1543.5000000,-1635.8000500,13.2000000,0.0000000,0.0000000,0.0000000); //object(fenceshit3) (2)
	CreateDynamicObject(1251,1537.5000000,-1678.0000000,12.5000000,0.0000000,0.0000000,0.0000000); //object(smashbar) (3)
	CreateDynamicObject(1251,1537.5000000,-1672.5000000,12.5000000,0.0000000,0.0000000,0.0000000); //object(smashbar) (3)
	CreateDynamicObject(1251,1537.5000000,-1667.3000500,12.5000000,0.0000000,0.0000000,0.0000000); //object(smashbar) (3)
	CreateDynamicObject(3460,1548.4000200,-1632.5000000,16.6000000,0.0000000,0.0000000,0.0000000); //object(vegaslampost) (1)
	CreateDynamicObject(3460,1560.9000200,-1620.4000200,16.8000000,0.0000000,0.0000000,0.0000000); //object(vegaslampost) (2)
	//Bus urbano TRABAJO.
...
//Policia federal INTERIOR.
	...
	//Trabajo: Taxista.
	CreateDynamicObject(996,1804.0999800,-1884.5000000,13.3000000,0.0000000,0.0000000,0.0000000); //object(lhouse_barrier1) (1)
	CreateDynamicObject(996,1791.9000200,-1884.5000000,13.3000000,0.0000000,0.0000000,0.0000000); //object(lhouse_barrier1) (2)
	CreateDynamicObject(996,1781.0999800,-1884.5000000,13.3000000,0.0000000,0.0000000,0.0000000); //object(lhouse_barrier1) (3)
	CreateDynamicObject(996,1798.8000500,-1936.6999500,13.3000000,0.0000000,0.0000000,0.0000000); //object(lhouse_barrier1) (4)
	CreateDynamicObject(996,1788.0999800,-1936.6999500,13.3000000,0.0000000,0.0000000,0.0000000); //object(lhouse_barrier1) (5)
	CreateDynamicObject(996,1779.0000000,-1936.6999500,13.3000000,0.0000000,0.0000000,0.0000000); //object(lhouse_barrier1) (6)
	CreateDynamicObject(996,1752.9000200,-1905.4000200,13.3000000,0.0000000,0.0000000,0.0000000); //object(lhouse_barrier1) (7)
	CreateDynamicObject(996,1752.8000500,-1891.9000200,13.3000000,0.0000000,0.0000000,0.0000000); //object(lhouse_barrier1) (8)
	CreateDynamicObject(996,1753.1999500,-1898.5000000,13.3000000,0.0000000,0.0000000,0.0000000); //object(lhouse_barrier1) (9)
	CreateDynamicObject(3460,1777.0000000,-1935.0999800,16.7000000,0.0000000,0.0000000,0.0000000); //object(vegaslampost) (1)
	CreateDynamicObject(3460,1787.0000000,-1935.0999800,16.7000000,0.0000000,0.0000000,0.0000000); //object(vegaslampost) (2)
	CreateDynamicObject(3460,1797.0000000,-1935.4000200,16.7000000,0.0000000,0.0000000,0.0000000); //object(vegaslampost) (3)
	CreateDynamicObject(3460,1806.5999800,-1935.0999800,16.7000000,0.0000000,0.0000000,0.0000000); //object(vegaslampost) (4)
	CreateDynamicObject(1319,1811.5999800,-1894.6999500,13.1000000,0.0000000,0.0000000,0.0000000); //object(ws_ref_bollard) (1)
	CreateDynamicObject(1319,1811.6999500,-1884.5000000,13.1000000,0.0000000,0.0000000,0.0000000); //object(ws_ref_bollard) (2)
	CreateDynamicObject(1319,1774.3000500,-1884.5000000,13.1000000,0.0000000,0.0000000,0.0000000); //object(ws_ref_bollard) (3)
	CreateDynamicObject(1319,1777.3000500,-1883.6999500,22.2000000,0.0000000,0.0000000,0.0000000); //object(ws_ref_bollard) (4)
	CreateDynamicObject(1215,1811.5999800,-1894.9000200,13.1000000,0.0000000,0.0000000,0.0000000); //object(bollardlight) (1)
	CreateDynamicObject(1215,1811.6999500,-1884.3000500,13.1000000,0.0000000,0.0000000,0.0000000); //object(bollardlight) (2)
	CreateDynamicObject(1215,1774.5000000,-1884.5000000,13.1000000,0.0000000,0.0000000,0.0000000); //object(bollardlight) (3)
	CreateDynamicObject(1215,1774.1999500,-1906.5999800,13.1000000,0.0000000,0.0000000,0.0000000); //object(bollardlight) (4)
	CreateDynamicObject(1215,1769.5000000,-1907.0000000,13.1000000,0.0000000,0.0000000,0.0000000); //object(bollardlight) (5)
	CreateDynamicObject(1215,1752.3000500,-1905.1999500,13.1000000,0.0000000,0.0000000,0.0000000); //object(bollardlight) (6)
	CreateDynamicObject(1215,1752.4000200,-1901.0000000,13.1000000,0.0000000,0.0000000,0.0000000); //object(bollardlight) (7)
	CreateDynamicObject(1215,1752.3000500,-1896.3000500,13.1000000,0.0000000,0.0000000,0.0000000); //object(bollardlight) (8)
	CreateDynamicObject(1215,1752.4000200,-1892.0999800,13.1000000,0.0000000,0.0000000,0.0000000); //object(bollardlight) (9)
	CreateDynamicObject(1215,1774.4000200,-1935.6999500,13.1000000,0.0000000,0.0000000,0.0000000); //object(bollardlight) (10)
	CreateDynamicObject(1215,1806.8000500,-1912.9000200,13.1000000,0.0000000,0.0000000,0.0000000); //object(bollardlight) (11)
	CreateDynamicObject(1215,1806.8000500,-1922.0999800,13.1000000,0.0000000,0.0000000,0.0000000); //object(bollardlight) (12)
	CreateDynamicObject(1215,1774.1999500,-1925.0999800,13.1000000,0.0000000,0.0000000,0.0000000); //object(bollardlight) (13)
	CreateDynamicObject(1215,1774.1999500,-1914.5999800,13.1000000,0.0000000,0.0000000,0.0000000); //object(bollardlight) (14)
	CreateDynamicObject(11489,1764.3000500,-1884.8000500,12.6000000,0.0000000,0.0000000,0.0000000); //object(dam_statues) (1)
	CreateDynamicObject(13758,1755.6999500,-1874.4000200,46.4000000,0.0000000,0.0000000,0.0000000); //object(radarmast1_lawn01) (1)
	CreateDynamicObject(3578,1801.6999500,-1920.0999800,11.6700000,0.0000000,0.0000000,0.0000000); //object(dockbarr1_la) (1)
	CreateDynamicObject(3578,1791.5000000,-1920.0999800,11.6700000,0.0000000,0.0000000,0.0000000); //object(dockbarr1_la) (2)
	CreateDynamicObject(3578,1781.1999500,-1920.0999800,11.6700000,0.0000000,0.0000000,0.0000000); //object(dockbarr1_la) (3)
	CreateDynamicObject(3578,1774.9000200,-1920.0999800,11.6700000,0.0000000,0.0000000,0.0000000); //object(dockbarr1_la) (4)
	CreateDynamicObject(3578,1811.4000200,-1889.5999800,11.6700000,0.0000000,0.0000000,90.0000000); //object(dockbarr1_la) (5)
	...
	//Banco intNuevo
	CreateObject(14789, 824.60065, -2075.38892, 99994.63281,   0.00000, 0.00000, 0.00000);
	CreateObject(18070, 838.73138, -2091.52905, 99990.80469,   0.00000, 0.00000, 0.00000);
	CreateObject(18070, 838.11713, -2079.99609, 99990.80469,   0.00000, 0.00000, 180.00000);
	CreateObject(18070, 843.75000, -2085.74878, 99990.80469,   0.00000, 0.00000, 90.00000);
	CreateObject(18070, 833.02057, -2085.57715, 99990.80469,   0.00000, 0.00000, -90.00000);
	CreateObject(2596, 836.40173, -2092.76050, 99991.46094,   0.00000, 0.00000, 140.09993);
	CreateObject(7191, 828.31079, -2069.35352, 99989.75781,   0.00000, 0.00000, 0.00000);
	CreateObject(7191, 828.31079, -2069.35352, 99997.01563,   0.00000, 0.00000, 0.00000);
	CreateObject(3857, 828.33160, -2080.28003, 99994.50000,   0.00000, 0.00000, -45.00000);
	CreateObject(3857, 828.33160, -2087.71143, 99994.50000,   0.00000, 0.00000, -45.00000);
	CreateObject(3857, 828.33160, -2072.87451, 99994.50000,   0.00000, 0.00000, -45.00000);
	CreateObject(3857, 828.33160, -2065.87671, 99994.50000,   0.00000, 0.00000, -45.00000);
	CreateObject(3462, 845.64307, -2099.24683, 99991.84375,   0.00000, 0.00000, -90.00000);
	CreateObject(3462, 842.74713, -2099.26904, 99991.84375,   0.00000, 0.00000, -90.00000);
	CreateObject(2942, 848.17468, -2075.27637, 99990.98438,   0.00000, 0.00000, -90.00000);
	CreateObject(2942, 848.22412, -2076.39624, 99990.98438,   0.00000, 0.00000, -90.00000);
	CreateObject(1886, 877.23596, -2140.32373, 100006.79688,   32.16000, -17.88000, -43.86000);
//BANCO INT
/*
...
*/
	// MECANICO
	CreateDynamicObject(6959,1266.8000000,-1682.6000000,12.5700000,0.0000000,0.0000000,0.0000000); //object(vegasnbball1) (1)
	CreateDynamicObject(6959,1225.5000000,-1642.6400000,12.5700000,0.0000000,0.0000000,0.0000000); //object(vegasnbball1) (2)
	CreateDynamicObject(6959,1266.8000000,-1603.3000000,12.5700000,0.0000000,0.0000000,0.0000000); //object(vegasnbball1) (3)
	CreateDynamicObject(6959,1225.5000000,-1682.5000000,12.5700000,0.0000000,0.0000000,0.0000000); //object(vegasnbball1) (4)
	CreateDynamicObject(6959,1266.8000000,-1642.6400000,12.5700000,0.0000000,0.0000000,0.0000000); //object(vegasnbball1) (5)
	CreateDynamicObject(6959,1225.5000000,-1603.3000000,12.5700000,0.0000000,0.0000000,0.0000000); //object(vegasnbball1) (7)
	CreateDynamicObject(8230,1276.8000000,-1618.0000000,14.5000000,0.0000000,0.0000000,179.9950000); //object(vgsbikeschl01) (1)
	CreateDynamicObject(8231,1276.9000000,-1618.3000000,14.3000000,0.0000000,0.0000000,180.0000000); //object(vgsbikeschl05) (1)
	CreateDynamicObject(2802,1269.5000000,-1624.2000000,12.9000000,0.0000000,0.0000000,0.0000000); //object(castable1) (1)
	CreateDynamicObject(2802,1271.6000000,-1624.0000000,12.9000000,0.0000000,0.0000000,0.0000000); //object(castable1) (2)
	CreateDynamicObject(1998,1269.5000000,-1611.2000000,12.6000000,0.0000000,0.0000000,180.0000000); //object(officedesk1l) (1)
	CreateDynamicObject(1663,1268.3000000,-1611.1000000,13.1000000,0.0000000,0.0000000,0.0000000); //object(swivelchair_b) (1)
	CreateDynamicObject(1721,1268.1000000,-1621.3000000,12.6000000,0.0000000,0.0000000,270.0000000); //object(est_chair1) (1)
	CreateDynamicObject(1721,1268.1000000,-1620.7000000,12.6000000,0.0000000,0.0000000,269.9950000); //object(est_chair1) (2)
	CreateDynamicObject(1721,1268.1000000,-1620.1000000,12.6000000,0.0000000,0.0000000,269.9950000); //object(est_chair1) (3)
	CreateDynamicObject(1721,1268.1000000,-1619.5000000,12.6000000,0.0000000,0.0000000,269.9950000); //object(est_chair1) (4)
	CreateDynamicObject(1721,1268.1000000,-1618.9000000,12.6000000,0.0000000,0.0000000,269.9950000); //object(est_chair1) (5)
	CreateDynamicObject(2128,1275.3000000,-1610.3000000,12.6000000,0.0000000,0.0000000,0.0000000); //object(cj_k1_tall_unit) (1)
	CreateDynamicObject(2128,1276.5000000,-1610.3000000,12.6000000,0.0000000,0.0000000,0.0000000); //object(cj_k1_tall_unit) (2)
	CreateDynamicObject(2128,1278.0000000,-1610.3000000,12.6000000,0.0000000,0.0000000,0.0000000); //object(cj_k1_tall_unit) (3)
	CreateDynamicObject(3051,1275.3000000,-1614.9000000,14.0000000,0.0000000,0.0000000,312.0000000); //object(lift_dr) (1)
	CreateDynamicObject(1491,1277.6000000,-1614.9000000,12.6000000,0.0000000,0.0000000,0.0000000); //object(gen_doorint01) (1)
000,0.0000000,90.0000000,90.0000000); //object(esc_step) (86)
	CreateDynamicObject(1656,1292.0000000,-1633.7000000,18.5000000,0.0000000,90.0000000,90.0000000); //object(esc_step) (87)
	CreateDynamicObject(1656,1294.2000000,-1633.7000000,18.9000000,90.0000000,0.0000000,0.0000000); //object(esc_step) (88)
	CreateDynamicObject(1656,1295.5000000,-1633.7000000,18.9000000,90.0000000,0.0000000,0.0000000); //object(esc_step) (89)
	CreateDynamicObject(1656,1296.8000000,-1633.7000000,18.9000000,90.0000000,0.0000000,0.0000000); //object(esc_step) (90)
	CreateDynamicObject(3877,1286.7000000,-1650.9000000,14.2000000,0.0000000,0.0000000,0.0000000); //object(sf_rooflite) (1)
	CreateDynamicObject(3877,1278.6000000,-1651.0000000,14.2000000,0.0000000,0.0000000,0.0000000); //object(sf_rooflite) (2)
	CreateDynamicObject(3877,1287.1000000,-1633.9000000,14.2000000,0.0000000,0.0000000,0.0000000); //object(sf_rooflite) (3)
	CreateDynamicObject(3877,1278.6000000,-1634.2000000,14.2000000,0.0000000,0.0000000,0.0000000); //object(sf_rooflite) (4)
	CreateDynamicObject(3578,1278.5000000,-1639.9000000,11.8400000,0.0000000,0.0000000,90.0000000); //object(dockbarr1_la) (1)
	CreateDynamicObject(3578,1278.5000000,-1645.6000000,11.8400000,0.0000000,0.0000000,90.0000000); //object(dockbarr1_la) (2)
	CreateDynamicObject(2971,1238.7000000,-1609.7000000,12.5000000,0.0000000,0.0000000,0.0000000); //object(k_smashboxes) (1)
	CreateDynamicObject(2971,1241.2000000,-1609.4000000,12.5000000,0.0000000,0.0000000,0.0000000); //object(k_smashboxes) (2)
	CreateDynamicObject(2971,1239.8000000,-1608.9000000,13.3000000,0.0000000,0.0000000,0.0000000); //object(k_smashboxes) (3)
	CreateDynamicObject(2971,1243.4000000,-1609.3000000,12.5000000,0.0000000,0.0000000,0.0000000); //object(k_smashboxes) (4)
	CreateDynamicObject(2971,1241.9000000,-1607.7000000,13.9000000,0.0000000,0.0000000,0.0000000); //object(k_smashboxes) (5)
	CreateDynamicObject(2971,1245.3000000,-1608.5000000,12.5000000,0.0000000,0.0000000,0.0000000); //object(k_smashboxes) (6)
	CreateDynamicObject(12957,1250.8000000,-1611.0000000,13.4000000,0.0000000,0.0000000,0.0000000); //object(sw_pickupwreck01) (1)
	CreateDynamicObject(939,1215.6000000,-1636.2000000,15.0000000,0.0000000,0.0000000,0.0000000); //object(cj_df_unit) (1)
	CreateDynamicObject(939,1226.1000000,-1636.0000000,15.0000000,0.0000000,0.0000000,0.0000000); //object(cj_df_unit) (2)
	CreateDynamicObject(939,1226.3000000,-1614.2000000,15.0000000,0.0000000,0.0000000,0.0000000); //object(cj_df_unit) (3)
	CreateDynamicObject(939,1218.4000000,-1665.9000000,15.0000000,0.0000000,0.0000000,0.0000000); //object(cj_df_unit) (4)
	CreateDynamicObject(939,1229.2000000,-1666.4000000,15.0000000,0.0000000,0.0000000,0.0000000); //object(cj_df_unit) (5)
	CreateDynamicObject(939,1216.0000000,-1644.6000000,15.0000000,0.0000000,0.0000000,0.0000000); //object(cj_df_unit) (6)
	CreateDynamicObject(939,1228.1000000,-1644.2000000,15.0000000,0.0000000,0.0000000,0.0000000); //object(cj_df_unit) (7)

	/*                                                              MAPPING*/
	
	CheckForFile("Archivos de Coches/Coches/");
	CheckForFile("Archivos de Coches/Coches/Propietarios/");
...
public OnPlayerRequestClass(playerid, classid)
{
	...
	/* Registro y inicio.*/
	if(fexist(Archivo)) { Dialog(playerid, Inicio,DIALOG_STYLE_INPUT,"{FFFFFF}Inicio al servidor.","{FFFFFF}Bienvenido de nuevo al servidor\nPor favor Ingresa  con tu {0095FF}contraseña y Inícia","Iniciar","Salir");
...
TextDrawFont(TextVelo2[playerid], 3);
TextDrawLetterSize(TextVelo2[playerid], 0.310000, 1.499999);
TextDrawColor(TextVelo2[playerid], 3315400);
TextDrawSetOutline(TextVelo2[playerid], 0);
TextDrawSetProportional(TextVelo2[playerid], 1);
TextDrawSetShadow(TextVelo2[playerid], 1);
TextDrawSetSelectable(TextVelo2[playerid], 0);
	
TextGas[playerid] = TextDrawCreate(600.000000, 425.000000, "100");
...
public OnPlayerSpawn(playerid)
{
	...
	if(MurioJP[playerid] == 1) {
	PPos(playerid,1164.6,-1323.9,13.9);
	MurioJP[playerid] = 0;
	}
...
public OnPlayerDeath(playerid, killerid, reason)
{
    ...
    {
     SendClientMessage(killerid,0xAA3333AA, "¿Qué haces? , No se puede hacer CK en éste servidor!");
    }
	return 1;
}
...
if(Aceptollamada[playerid] == 1) {
new String[150];
format(String,sizeof(String),"[Llamada telefónica] escucha: %s.",text);
Mensaje(Llamaaluser[playerid],COLOR_BLANCO,String);
format(String,sizeof(String),"[Llamada telefónica] tu dices: %s.",text);
...
public OnPlayerEnterVehicle(playerid, vehicleid, ispassenger)
{
	...
	if(vehicleid == 422) { if(ispassenger) { if(Pidiotaxi[playerid] == 1) { TimerTaxi[playerid] = SetTimerEx("FichaT", 15000, false, "i", playerid); EstaenTaxi[playerid] = 1; } } }
...
new String[120];
format(String,sizeof(String),"%d",Gasolina[Vehi]);
TextDrawSetString(TextGas[Usuario],String);
...
public OnPlayerStateChange(playerid, newstate, oldstate)
{
	...
	{
 	new dialog_string[308];

	if(VehicleStatistics[vehicleid][vehicle_onsale] == 1)
	{
	new String[300] = "{FFE600}!«\n\n{009AFF}-{FFFFFF} Modelo:{555555} %s{FFFFFF} «\n{009AFF}-{FFFFFF} Precio: {00BE13}$%d {FFFFFF}«\n¿Quiéres comprar el vehículo?";
	format(dialog_string, sizeof(dialog_string),String, GetVehicleName(vehicleid), VehicleStatistics[vehicleid][vehicle_price]);
	ShowPlayerDialog(playerid, 1337, DIALOG_STYLE_MSGBOX, "Compra de coche", dialog_string, "Sí", "No");
	return 1;
	}
...
*/
	...
	Mensaje(playerid,COLOR_BLANCO,"Se te ha pagado{00C71E} $200{FFFFFF} , por entregar mercadería.");
	Quitarcoche(playerid);
	...
	Mensaje(playerid,COLOR_BLANCO,"Se te ha abonado{00C71E} $230{FFFFFF}, Por el trabajo.");
...
/*	if(LineaA[playerid] == 1) {
	...
	Mensaje(playerid,COLOR_BLANCO,"Se te ha abonado{00C71E} $190{FFFFFF}, Por el trabajo.");
	LineaA[playerid] = 0;
	...
	ControlP(playerid,0);
	SetTimerEx("BasureroParada", 10000, false, "i", playerid);
	}
...
stock LoadDealershipCars()
{
	...
	for(new i = 1; i < MAX_VEHICLES; i++)
	{
		format(file, sizeof(file), "Archivos de Coches/Coches/%d.cfg", i);
		...
		if(fexist(file) && INI_Open(file))
		{
		    if(GetVehicleModel(i) > 399)
		    {
				...
				format(file2, sizeof(file2), "Archivos de Coches/Coches/%d.cfg", i);
				frename(file, file2);
			}
			...
			vehicle = CreateVehicle(VehicleStatistics[i][vehicle_model], VehicleStatistics[i][vehicle_position][1], VehicleStatistics[i][vehicle_position][2], VehicleStatistics[i][vehicle_position][3], VehicleStatistics[i][vehicle_position][4], 1, 1, -1);
			format(string, sizeof(string), "{00C0FF}[EN VENTA]{FFFFFF}\n%s {00DA00}Precio:{FFFFFF} $%d", GetVehicleName(vehicle), VehicleStatistics[i][vehicle_price]);
...
stock LoadDealerships()
{
	...
	for(new i = 1; i < MAX_DEALERSHIPS; i++)
	{
		format(file, sizeof(file), "Archivos de Coches/%d.cfg", i);
		if(fexist(file) && INI_Open(file))
		{
			...
			{
			    case 0: { format(string, sizeof(string), "{00C0FF}[EN VENTA]{FFFFFF}\n%s {00DA00}Precio:{FFFFFF} $%d", DealershipStatistics[i][dealership_name], DealershipStatistics[i][dealership_price]); }
				case 1: { format(string, sizeof(string), "{00C0FF}%s{FFFFFF}\n\n{00C0FF}Propietario:{FFFFFF} %s", DealershipStatistics[i][dealership_name], DealershipStatistics[i][dealership_owner]); }
			}
...
stock SaveDealership(dealershipid)
{
	...
	if(dealership_taken[dealershipid] == true)
 	{
		format(file, sizeof(file), "Archivos de Coches/%d.cfg", dealershipid);
		if(INI_Open(file))
		{
...
public OnPlayerClickTextDraw(playerid, Text:clickedid)
{
	...
	} else Mensaje(playerid,COLOR_BLANCO,"Ya estas en una llama, utiliza /teléfono.");
    }
    else if(clickedid == TextC3)
    {
    if(Aceptollamada[playerid] == 1) {
	Aceptollamada[Llamaaluser[playerid]] = false;
	Mensaje(Llamaaluser[playerid],COLOR_BLANCO,"Se cortó la llamada.");
	...
	Aceptollamada[playerid] = false;
	Llamaaluser[playerid] = false;
	} else Mensaje(playerid,COLOR_BLANCO,"No estas en una llamada");
	}
...
public OnPlayerUpdate(playerid)
{
	...
	Mensaje(i,COLOR_BLANCO,"ATENCIÓN: Se ha recibido un llamado de emergencia, ve al lugar.");
    MedicoPedido = 1;
	}
	}
	}
	...
	Mensaje(playerid,COLOR_BLANCO,"Estabas inconciente, pero los médicos te reanimaron");
	}
	}
	...
	PPos(playerid,246.8000000,62.7000000,1004.2999900);
	cargarjugador(playerid);
	Mensaje(playerid,COLOR_BLANCO,"{00A6FF}-Policía{FFFFFF}: Vete y si vuelves será el doble!");
	}
...
new IDLM,PidioMecanico[MAX_PLAYERS],LMecanico;*/
...
new String[150];
format(String,sizeof(String),"Has pagado $%d por %d litros de gasolína.",GasolinaP*strval(inputtext),strval(inputtext));
Mensaje(playerid,COLOR_BLANCO,String);
CargarGas[playerid] = strval(inputtext);
TanqueTimer = SetTimerEx("Llenartanque", 2000, true, "i", playerid);
}
}
if(dialogid == D_Vendercochea) { if(response) {
			...
			vehicleid = CreateVehicle(ModeloC[playerid], (position[1] + (7.5 * floatsin(-position[4], degrees))), (position[2] + (7.5 * floatcos(position[4], degrees))), position[3], position[4], 1, 1, -1);
	        ...
	        if(GetPVarInt(playerid, "AmountOfCars") >= MAX_OWNABLE_CARS)
				return SendClientMessage(playerid, -1, "<! {FFFFFF} Usted ya tiene la cantidad máxima de vehículos !"), RemovePlayerFromVehicle(playerid);

			RemovePlayerFromVehicle(IDMIA[playerid]);
			SendClientMessage(playerid, color_white, string);
			SendClientMessage(playerid, color_white, "El vehículo ha sido entragado a usted.");
			...
			SetPVarInt(playerid, var, ModeloC[playerid]);
			...
			format(var, sizeof(var), "ValorC_%d", GetPVarInt(playerid, "AmountOfCars"));
			SetPVarInt(playerid, var, price);
			format(var, sizeof(var), "Comprado_%d", GetPVarInt(playerid, "AmountOfCars"));
			SetPVarInt(playerid, var, 1);
			...
			strmid(owner[vehicleid], playername, 0, strlen(playername), 255);
			SavePlayerVehicleData(playerid);
			DeleteVehicle(IDMIA[playerid],Slotventa[playerid]);
}
}

if(dialogid == Dartrabajo) { if(response) {
if(listitem == 0) { Dialog(playerid,Dartrabajo2,DIALOG_STYLE_INPUT,"- Menú Job's.","Inserta la ID del usuario, que\n quieres dar el trabajo.","Elegir","Salir"); NumTrabajo[playerid] = 1; }
if(listitem == 1) { Dialog(playerid,Dartrabajo2,DIALOG_STYLE_INPUT,"- Menú Job's.","Inserta la ID del usuario, que\n quieres dar el trabajo.","Elegir","Salir"); NumTrabajo[playerid] = 2; }
if(listitem == 2) { Dialog(playerid,Dartrabajo2,DIALOG_STYLE_INPUT,"- Menú Job's.","Inserta la ID del usuario, que\n quieres dar el trabajo.","Elegir","Salir"); NumTrabajo[playerid] = 3; }
if(listitem == 3) { Dialog(playerid,Dartrabajo2,DIALOG_STYLE_INPUT,"- Menú Job's.","Inserta la ID del usuario, que\n quieres dar el trabajo.","Elegir","Salir"); NumTrabajo[playerid] = 4; }
if(listitem == 4) { Dialog(playerid,Dartrabajo2,DIALOG_STYLE_INPUT,"- Menú Job's.","Inserta la ID del usuario, que\n quieres dar el trabajo.","Elegir","Salir"); NumTrabajo[playerid] = 5; }
if(listitem == 5) { Dialog(playerid,Dartrabajo2,DIALOG_STYLE_INPUT,"- Menú Job's.","Inserta la ID del usuario, que\n quieres dar el trabajo.","Elegir","Salir"); NumTrabajo[playerid] = 6; }
if(listitem == 6) { Dialog(playerid,Dartrabajo2,DIALOG_STYLE_INPUT,"- Menú Job's.","Inserta la ID del usuario, que\n quieres dar el trabajo.","Elegir","Salir"); NumTrabajo[playerid] = 7; }
if(listitem == 7) { Dialog(playerid,Dartrabajo2,DIALOG_STYLE_INPUT,"- Menú Job's.","Inserta la ID del usuario, que\n quieres dar el trabajo.","Elegir","Salir"); NumTrabajo[playerid] = 8; }
} }
...
if(listitem == 0) { Dialog(playerid,D_DARFAC2,DIALOG_STYLE_INPUT,"- Menú facción.","Inserta la ID del usuario, que\n quieres dar la facción.","Elegir","Salir"); Facciondar[playerid] = 1; }
if(listitem == 1) { Dialog(playerid,D_DARFAC2,DIALOG_STYLE_INPUT,"- Menú facción.","Inserta la ID del usuario, que\n quieres dar la facción.","Elegir","Salir"); Facciondar[playerid] = 2; }
if(listitem == 2) { Dialog(playerid,D_DARFAC2,DIALOG_STYLE_INPUT,"- Menú facción.","Inserta la ID del usuario, que\n quieres dar la facción.","Elegir","Salir"); Facciondar[playerid] = 3; }
if(listitem == 3) { Dialog(playerid,D_DARFAC2,DIALOG_STYLE_INPUT,"- Menú facción.","Inserta la ID del usuario, que\n quieres dar la facción.","Elegir","Salir"); Facciondar[playerid] = 4; }
if(listitem == 4) { Dialog(playerid,D_DARFAC2,DIALOG_STYLE_INPUT,"- Menú facción.","Inserta la ID del usuario, que\n quieres dar la facción.","Elegir","Salir"); Facciondar[playerid] = 5; }
if(listitem == 5) { Dialog(playerid,D_DARFAC2,DIALOG_STYLE_INPUT,"- Menú facción.","Inserta la ID del usuario, que\n quieres dar la facción.","Elegir","Salir"); Facciondar[playerid] = 6; }
if(listitem == 6) { Dialog(playerid,D_DARFAC2,DIALOG_STYLE_INPUT,"- Menú facción.","Inserta la ID del usuario, que\n quieres dar la facción.","Elegir","Salir"); Facciondar[playerid] = 7; }
if(listitem == 7) { Dialog(playerid,D_DARFAC2,DIALOG_STYLE_INPUT,"- Menú facción.","Inserta la ID del usuario, que\n quieres dar la facción.","Elegir","Salir"); Facciondar[playerid] = 8; }
if(listitem == 8) { Dialog(playerid,D_DARFAC2,DIALOG_STYLE_INPUT,"- Menú facción.","Inserta la ID del usuario, que\n quieres dar la facción.","Elegir","Salir"); Facciondar[playerid] = 9; }
if(listitem == 9) { Dialog(playerid,D_DARFAC2,DIALOG_STYLE_INPUT,"- Menú facción.","Inserta la ID del usuario, que\n quieres dar la facción.","Elegir","Salir"); Facciondar[playerid] = 10; }
} }
if(dialogid == D_DARFAC2) { if(response) {
new Nombre[MAX_PLAYER_NAME],Nombre2[MAX_PLAYER_NAME],String[300];
PI[strval(inputtext)][Faccion] = Facciondar[playerid];
PI[strval(inputtext)][Rangof] = 6;
GetPlayerName(playerid,Nombre,sizeof(Nombre));
GetPlayerName(strval(inputtext),Nombre2,sizeof(Nombre));
format(String,sizeof(String),"{FF0000}<!>{FFFFFF} El administrador {0084FF}%s{FFFFFF} dio una facción a {0084FF}%s{FFFFFF}. Facción ID: {FF8900}%d",Nombre,Nombre2,Facciondar[playerid]);
Mensaje(playerid,COLOR_BLANCO,String); Mensaje(strval(inputtext),COLOR_BLANCO,String);
}
}
...
} else Mensaje(playerid,COLOR_BLANCO,"No tienes suficiente dinero para comprar ésta arma.");
...
Dialog(playerid,D_ESCRIBERUBRO,DIALOG_STYLE_INPUT,"- [Vehículos]","Escribe tu anuncio. - MÁX 128 Caracteres.","Listo","Salir");
}
if(listitem == 1) {
EligioRubro[playerid] = 2;
Dialog(playerid,D_ESCRIBERUBRO,DIALOG_STYLE_INPUT,"- [Propiedad]","Escribe tu anuncio. - MÁX 128 Caracteres.","Listo","Salir");
}
if(listitem == 2) {
EligioRubro[playerid] = 3;
Dialog(playerid,D_ESCRIBERUBRO,DIALOG_STYLE_INPUT,"- [Compra-Venta]","Escribe tu anuncio. - MÁX 128 Caracteres.","Listo","Salir");
}
if(listitem == 3) {
EligioRubro[playerid] = 4;
Dialog(playerid,D_ESCRIBERUBRO,DIALOG_STYLE_INPUT,"- [Otros]","Escribe tu anuncio. - MÁX 128 Caracteres.","Listo","Salir");
}
}
}
...
new String[200];
if(!strlen(inputtext)) return Dialog(playerid,D_ESCRIBERUBRO,DIALOG_STYLE_INPUT,"- [Vehículos]","Escribe tu anuncio. - MÁX 128 Caracteres.","Listo","Salir");
format(String,200,"{00FF2B}-{FFFFFF} [Vehículos] {009EFF}%s {FFFFFF}- Contacto: {009EFF}%d.",inputtext,PI[playerid][Telefono]);
...
new String[200];
if(!strlen(inputtext)) return Dialog(playerid,D_ESCRIBERUBRO,DIALOG_STYLE_INPUT,"- [Propiedad]","Escribe tu anuncio. - MÁX 128 Caracteres.","Listo","Salir");
format(String,sizeof(String),"{00FF2B}-{FFFFFF} [Propiedad] {009EFF}%s {FFFFFF}- Contacto: {009EFF}%d.",inputtext,PI[playerid][Telefono]);
SendClientMessageToAll(COLOR_BLANCO,String);
EligioRubro[playerid] = 0;
GivePlayerMoney(playerid,-20);
}
...
new String[200];
if(!strlen(inputtext)) return Dialog(playerid,D_ESCRIBERUBRO,DIALOG_STYLE_INPUT,"- [Compra-Venta]","Escribe tu anuncio. - MÁX 128 Caracteres.","Listo","Salir");
format(String,sizeof(String),"{00FF2B}-{FFFFFF} [Compra-Venta] {009EFF}%s {FFFFFF}- Contacto: {009EFF}%d.",inputtext,PI[playerid][Telefono]);
SendClientMessageToAll(COLOR_BLANCO,String);
EligioRubro[playerid] = 0;
GivePlayerMoney(playerid,-20);
}
...
new String[200];
if(!strlen(inputtext)) return Dialog(playerid,D_ESCRIBERUBRO,DIALOG_STYLE_INPUT,"- [Otros]","Escribe tu anuncio. - MÁX 128 Caracteres.","Listo","Salir");
format(String,sizeof(String),"{00FF2B}-{FFFFFF} [Otros] {009EFF}%s {FFFFFF}- Contacto: {009EFF}%d.",inputtext,PI[playerid][Telefono]);
SendClientMessageToAll(COLOR_BLANCO,String);
EligioRubro[playerid] = 0;
GivePlayerMoney(playerid,-20);
}
...
{
Dialog(playerid,D_SerieBanco,DIALOG_STYLE_INPUT,"Serie","Ingresa el código de serie.","Ingresar","Salir");
...
if(dialogid == 1337)
	{
	    if(response)
	    {
			...
			if(GetPlayerMoney(playerid) < price)
			    return SendClientMessage(playerid, -1,"<! {FFFFFF}No tienes dinero suficiente para comprar éste vehículo."), RemovePlayerFromVehicle(playerid);
			...
			if(GetPVarInt(playerid,"Model_1") == 0) {
			...
			RemovePlayerFromVehicle(playerid);
			vehicleid = CreateVehicle(model, (position[1] + (7.5 * floatsin(-position[4], degrees))), (position[2] + (7.5 * floatcos(position[4], degrees))), position[3], position[4], 1, 1, -1);
			format(string, sizeof(string), "Gracias por comprar en {00B3FF}%s.", DealershipStatistics[dealershipid][dealership_name]);
			SendClientMessage(playerid, color_white, string);
			SendClientMessage(playerid, color_white, "El vehículo ha sido entragado a usted.");

			SetPVarInt(playerid, "AmountOfCars", GetPVarInt(playerid, "AmountOfCars") + 1);

			format(var, sizeof(var), "Model_1", GetPVarInt(playerid, "AmountOfCars"));
			SetPVarInt(playerid, var, model);

			format(var, sizeof(var), "X_1", GetPVarInt(playerid, "AmountOfCars"));
			SetPVarFloat(playerid, var, (position[1] + (7.5 * floatsin(-position[4], degrees))));

			format(var, sizeof(var), "Y_1", GetPVarInt(playerid, "AmountOfCars"));
			SetPVarFloat(playerid, var, (position[2] + (7.5 * floatcos(position[4], degrees))));

			format(var, sizeof(var), "Z_1", GetPVarInt(playerid, "AmountOfCars"));
			SetPVarFloat(playerid, var, position[3]);

			format(var, sizeof(var), "Angle_1", GetPVarInt(playerid, "AmountOfCars"));
			SetPVarFloat(playerid, var, position[4]);

			format(var, sizeof(var), "Carkey_1", GetPVarInt(playerid, "AmountOfCars"));
			SetPVarInt(playerid, var, vehicleid);

			format(var, sizeof(var), "Paintjob_1", GetPVarInt(playerid, "AmountOfCars"));
			SetPVarInt(playerid, var, -1);

			format(var, sizeof(var), "ValorC_1", GetPVarInt(playerid, "AmountOfCars"));
			SetPVarInt(playerid, var, price);
			format(var, sizeof(var), "Comprado_1", GetPVarInt(playerid, "AmountOfCars"));
			SetPVarInt(playerid, var, 1);

			strmid(owner[vehicleid], playername, 0, strlen(playername), 255);
			SavePlayerVehicleData(playerid);
   			}
   			else if(GetPVarInt(playerid,"Model_2") == 0) {
			...
			format(var, sizeof(var), "Model_2", GetPVarInt(playerid, "AmountOfCars"));
			SetPVarInt(playerid, var, model);

			format(var, sizeof(var), "X_2", GetPVarInt(playerid, "AmountOfCars"));
			SetPVarFloat(playerid, var, (position[1] + (7.5 * floatsin(-position[4], degrees))));

			format(var, sizeof(var), "Y_2", GetPVarInt(playerid, "AmountOfCars"));
			SetPVarFloat(playerid, var, (position[2] + (7.5 * floatcos(position[4], degrees))));

			format(var, sizeof(var), "Z_2", GetPVarInt(playerid, "AmountOfCars"));
			SetPVarFloat(playerid, var, position[3]);

			format(var, sizeof(var), "Angle_2", GetPVarInt(playerid, "AmountOfCars"));
			SetPVarFloat(playerid, var, position[4]);

			format(var, sizeof(var), "Carkey_2", GetPVarInt(playerid, "AmountOfCars"));
			SetPVarInt(playerid, var, vehicleid);

			format(var, sizeof(var), "Paintjob_2", GetPVarInt(playerid, "AmountOfCars"));
			SetPVarInt(playerid, var, -1);

			format(var, sizeof(var), "ValorC_2", GetPVarInt(playerid, "AmountOfCars"));
			SetPVarInt(playerid, var, price);
			format(var, sizeof(var), "Comprado_2", GetPVarInt(playerid, "AmountOfCars"));
			SetPVarInt(playerid, var, 1);
...
strcat(String,"A la hora de conducir, tendremos que saber en qué carril tenemos\n");
strcat(String,"que conducir. Ya que si fuera el erroneo podría pasar un accidente.\n");
strcat(String,"Siempre se elige el carril derecho, además se debe respetar un lími-\n");
strcat(String,"te de velocidad máxima en la {FFEB00}ciudad. 40km/h - Ruta: 120km/h{FFFFFF}.\n");
strcat(String,"{FFEB00}Caso de emergencia:{FFFFFF} En caso de que si un vehículo de emergencia qui-\n");
strcat(String,"siera pasar , debemos cederle paso, ya que tenemos una obligación de hacerlo.");
Dialog(playerid,D_LicenciaV2,DIALOG_STYLE_MSGBOX,"Página Nº2.",String,"Siguiente","Anterior");
}
else
{
new String[600];
strcat(String,"\t\t{FFFFFF}- {FFEB00}TITULO:{FFFFFF} Iniciamiento del vehículo.\n\n");
strcat(String,"Lo primero de todo es saber Cómo es el funcionamiento del vehículo.\n");
strcat(String,"Para encender el motor: Ingresamos la llave correcta al vehículo.\n");
strcat(String,"Y le hacemos un giro de 180º.((IC:/Prender)).\n");
strcat(String,"Las luces: A cierta hora del día se tiene que tener las luces del -\n");
strcat(String,"Vehículo prendidas (Traseras y delantera). {FFEB00}¿A qué hora?{FFFFFF}\n");
strcat(String,"A la hora de la noche (20hs - 9hs.)\n");
Dialog(playerid,D_LicenciaV1,DIALOG_STYLE_MSGBOX,"Página Nº1.",String,"Siguiente","");
}
}
...
strcat(String,"Estado del vehículo: El vehículo en el que conduces tiene que estar\n");
strcat(String,"en perfectas condiciones, ya que sino la policía te dará una multa.\n");
strcat(String,"Fin del librode estudios, ahora viene el examen y una ves terminado");
strcat(String,"Se te pondrá en prueba tu capacidad para conducir.");
Dialog(playerid,D_LicenciaV3,DIALOG_STYLE_MSGBOX,"Página Nº3 /FINAL.",String,"Terminar","");
}
else
{
}
...
if(listitem == 2) {
Mensaje(playerid,COLOR_BLANCO,"Recuerda que tienes que tener los tres examenes aprobados.");
Dialog(playerid,D_LicenciaV5,DIALOG_STYLE_LIST,"¿A qué velocidad se\n puede conducir en la ciudad?\n","- 120km/h\n- 30km/h\n- 60km/h ","Siguiente","");
}
}
}
...
if(listitem == 1) {
Dialog(playerid,D_LicenciaV6,DIALOG_STYLE_LIST,"¿Si un vehículo de emergencia se acerca\n qué debo hacer?\n","- Que espere como todos los conductores.\n- Darle paso.\n- Lo choco. ","Terminar","");
}
if(listitem == 2) {
Mensaje(playerid,COLOR_BLANCO,"Has desaprobado el examen de licencia. Vuelve a intentar otra ves.");
}
...
if(listitem == 1) {
Mensaje(playerid,COLOR_BLANCO,"Subete al coche afuera y sigue la los checkpoints. El vehículo no puede estar menos del 70% roto.");
...
Mensaje(i,COLOR_BLANCO,"{FFE600}Servicio:{FFFFFF} Atención alguien ha llamado a la policía. Ve al lugar indicado.");
}
}
}
if(listitem == 1) {
...
if(PI[i][Faccion] == 3) {
Checkpoint(i,POLX,POLY,POLZ,5.0);
Mensaje(i,COLOR_BLANCO,"{FFE600}Servicio:{FFFFFF} Atención alguien ha llamado a la ambulancia. Ve al lugar indicado.");
MedicoPedido = 1;
}
}
}
...
Dialog(playerid,D_LSERVICIO,DIALOG_STYLE_LIST,"Elige tu servicio.","- Policía.\n- Médicos.","Elegir","Salir");
...
{
if(PI[i][Faccion] == 5) {
Mensaje(i,COLOR_BLANCO,"{FFE600}Servicio:{FFFFFF} Atención se ha pedido un servicio.Ve al lugar.");
...
if(!strlen(inputtext)) return Dialog(playerid, Registro,DIALOG_STYLE_INPUT,"Bienvenido","{555555}Zona Sur{FFFFFF}Roleplay\n\n Ingresá una contraseña y inicia.","Iniciar","");
new User[MAX_PLAYER_NAME],Archivo[MAX_PLAYERS];
GetPlayerName(playerid, User, sizeof(User));
format(Archivo,sizeof(Archivo),"Archivos de usuarios/%s.ini",User);
dini_Create(Archivo);/*
...
if(dialogid == Inicio) {
if(!response) return Kick(playerid);
if (!strlen(inputtext)) return Dialog(playerid, Inicio,DIALOG_STYLE_INPUT,"Bienvenido","{555555}Zona Sur{FFFFFF}Roleplay\n\n Ingresá tu contraseña y Inícia","Iniciar","");
...
if(response) {
if(listitem == 0) {
Mensaje(playerid,COLOR_BLANCO,"- Comandos: {009EFF}/Créditos  /Cuenta /reglas /Consulta /dConsulta /AConsulta");
Mensaje(playerid,COLOR_BLANCO,"-- Comandos: {009EFF}/Entrar /salir /b /me do /g /obtenerpasaporte /obtenerdni");
Mensaje(playerid,COLOR_BLANCO,"--- Comandos: {009EFF}/comprar /consumir /agarrarproducto /masinfo /elegirropa");
Mensaje(playerid,COLOR_BLANCO,"---- Comandos: {009EFF}/ComprarDrogas /armar (Porro). /Teléfono /Atender /Minumero");
Mensaje(playerid,COLOR_BLANCO,"----- Comandos: {009EFF}/LLenartanque /Casco /mercadonegro /obtenerlicencia");
Mensaje(playerid,COLOR_BLANCO,"------ Comandos: {009EFF}/obtenercuentabancaria /seriebanco /mostrarlicencia /ColocarAnuncio");
}
if(listitem == 1) { Mensaje(playerid,COLOR_BLANCO,"- Comandos: {009EFF}/Prender /Apagarcoche /ControlC /estacionar"); }
if(listitem == 2) { Mensaje(playerid,COLOR_BLANCO,"- Comandos: {009EFF}/Cajero (Tienes que estár en la posición del cajero.) - /Banco (Tienes que estár en el banco). "); }
if(listitem == 3) {
Mensaje(playerid,COLOR_BLANCO,"-Comandos: {009EFF}/Propiedad (Tienes que estár en la posición de tu propiedad.)  /comprarpropiedad.");
Mensaje(playerid,COLOR_BLANCO,"--Comandos: {009EFF}/SalirPropiedad {FFFFFF}(Tienes que estár en la posición de salida de tu propiedad.)  ");
}
if(listitem == 4) {
if(PI[playerid][Faccion] == 1) {
Mensaje(playerid,COLOR_BLANCO,"- Comandos: {009EFF}/R [Radio] - /F [FacciónOOC] - /M [Megáfono] - /Equipo - /Placa [ID]");
Mensaje(playerid,COLOR_BLANCO,"-- Comandos: {009EFF}/Desarmar [ID] - /Multar [ID] - /Refuerzos - /LocalizarSospechoso - /Detener ");
Mensaje(playerid,COLOR_BLANCO,"--- Comandos: {009EFF}Rango 5-6: /D [Departamento].");
Mensaje(playerid,COLOR_BLANCO,"---- Comandos: {009EFF}/Abrircarcel1 /Abrircarcel2 - /Cerrarcarcel1 /Cerrarcarcel2");
Mensaje(playerid,COLOR_BLANCO,"----- Comandos: {009EFF}/AbrirGarage - /CerrarGarage /Encarcelar"); }
	
else if(PI[playerid][Faccion] == 2) {
Mensaje(playerid,COLOR_BLANCO,"- Comandos: {009EFF}/R [Radio] - /F [FacciónOOC] ");  }

else if(PI[playerid][Faccion] == 3) {
Mensaje(playerid,COLOR_BLANCO,"- Comandos: {009EFF}/R [Radio] - /F [FacciónOOC] ");
Mensaje(playerid,COLOR_BLANCO,"-- Comandos: {009EFF}/PrepararJeringa - /Curar [ID] ");  }

else if(PI[playerid][Faccion] == 4) { Mensaje(playerid,COLOR_BLANCO,"- Comandos: {009EFF}/R [Radio] - /F [FacciónOOC] /SacardelFondoM /Fondos /ModificarImpuestos /ImpuestoP ");  }//Gobierno

else if(PI[playerid][Faccion] == 5) { Mensaje(playerid,COLOR_BLANCO,"- Comandos: {009EFF}/R [Radio] - /F [FacciónOOC] /RedactarNoticias. /Entrevistara");  }//LNTV

else if(PI[playerid][Faccion] == 6) {
	Mensaje(playerid,COLOR_BLANCO,"- Comandos: {009EFF}/R [Radio] - /F [FacciónOOC] /Colocarlopedido /ModificarCoche [id] /Remolcar");
	Mensaje(playerid,COLOR_BLANCO,"-- Comandos: {009EFF}Elevadorm1 /elevadorm2 /belevadorm1 /belevadorm2");   }//MECÁNICO
...
else if(PI[playerid][Trabajo] == 2) { //Píloto
Mensaje(playerid,COLOR_BLANCO,"Trabajo: Píloto.");
Mensaje(playerid,COLOR_BLANCO,"Sin comandos.");
}
...
Mensaje(playerid,COLOR_BLANCO,"- Comandos: {009EFF}/Aceptarservicio - /Cancelarservicio - /Tarifa [$0 a $20] ");
}
else if(PI[playerid][Trabajo] == 7) { //Bus Urbano
Mensaje(playerid,COLOR_BLANCO,"Trabajo: Bus URBANO (Chofer urbano).");
Mensaje(playerid,COLOR_BLANCO,"- Comandos: {009EFF}/Verficha - /UrbanoR ");
}
else if(PI[playerid][Trabajo] == 8) { //Ladrón
Mensaje(playerid,COLOR_BLANCO,"Trabajo: Ladrón.");
Mensaje(playerid,COLOR_BLANCO,"- Comandos: {009EFF}/Robaralapersona [ID] - /Venderlorobado ");
}
}
}
}
...
if(listitem == 3) {
Dialog(playerid,D_Transferirdinero,DIALOG_STYLE_INPUT,"- Transferencia.","- Coloca el número de serie\n del destinatario.","Siguiente","Salir");

}
 } }
/*if(dialogid == D_Transferirdinero2) { if(response) {
for(new i = 0; MAX_PLAYERS < i; i++)
{
if(IsPlayerConnected(i)) {
if(PI[i][NBanco] == strval(inputtext)) {
}
}
Dialog(playerid,D_Transferirdinero2,DIALOG_STYLE_INPUT,"- Transferencia.","- Coloca la cantidad que deseas transferir","Siguiente","Salir");

}
}

if(dialogid == D_Transferirdinero) { if(response) {
for(new i = 0; MAX_PLAYERS < i; i++)
{
if(IsPlayerConnected(i)) {
if(PI[i][NBanco] == strval(inputtext)) {
PI[i][Bancod] += Quieretransferir[playerid];
}
}
}
Mensaje(playerid,COLOR_BLANCO,"Has trasnferido ese dinero.");
}
}*/
	  		...
	  		switch(listitem)
	    	{
	    	    case 0:
	    	    {
	    	        for(new i = 0; i < MAX_HOUSES; i++)
					{
						...
						{
						if(PropiedadInfo[i][CCerrada] == 0) {
						    if(PropiedadInfo[i][IntID] == 1)
						    {
	    	 ...
	    	 /*   case 1:
				...
				case 1:
				{
				    ...
				    for(new i = 0; i < MAX_HOUSES; i++)
					{
                        ...
                        Update3DTextLabelText(PropiedadInfo[i][CasaLabel],0xFF0000FF,labelstring);
                        format(string,sizeof(string),"Vendiste tu propiedad a : %i",PropiedadInfo[i][Precio]/2);
				...
				{
				PropiedadInfo[i][CCerrada] = 0;
				}
				}
				Mensaje(playerid,COLOR_BLANCO,"Abriste ésta propiedad.");
				}
				case 4:
				{
				for(new i = 0; i < MAX_HOUSES; i++)
				...
				if(IsPlayerInRangeOfPoint(playerid,3.0,PropiedadInfo[i][CasaX],PropiedadInfo[i][CasaY],PropiedadInfo[i][CasaZ]))
				{
				PropiedadInfo[i][CCerrada] = 1;
				}
				}
				Mensaje(playerid,COLOR_BLANCO,"Cerraste ésta propiedad.");
				}
			}
		}
    }
...
Mensaje(playerid,COLOR_BLANCO,"[OOCINFO] -Has consumido cocaina - El efecto durará 3 minutos.");
...
Mensaje(playerid,COLOR_BLANCO,"[OOCINFO] Has consumido un porro - El efecto durará 3 minutos.");
...
format(String, sizeof(String), "[/Me] %s Enciende su vehículo.", Nombre);
ProxDetector(30.0, playerid, String, MoradoME,MoradoME,MoradoME,MoradoME,MoradoME);//
}
case 1: {
new String[150],Nombre[MAX_PLAYER_NAME]; GetPlayerName(playerid,Nombre,sizeof(Nombre));
format(String, sizeof(String), "[/Me] %s Intenta encender su vehículo pero falla.", Nombre);
ProxDetector(30.0, playerid, String, MoradoME,MoradoME,MoradoME,MoradoME,MoradoME);//
}
}
} }
...
if(DCoche(playerid)) {
Dialog(playerid,D_CONTROLVEHICULAR,DIALOG_STYLE_LIST,"Elige la acción.","- Luces\n- Puertas\n- Capo\n- Maletero.","Elegir","Salir");
}
return 1;
}
...
new Trabajos[30],Generos[20],String[700],User[MAX_PLAYER_NAME],FaccionN[35],Prestamos[40],String2[300];
GetPlayerName(playerid,User,sizeof(User));
if(PI[playerid][Trabajo] == 0) { Trabajos = "Sin trabajo"; }
else if(PI[playerid][Trabajo] == 1){ Trabajos = "Basurero"; }
else if(PI[playerid][Trabajo] == 2){ Trabajos = "Píloto"; }
...
else if(PI[playerid][Trabajo] == 8){ Trabajos = "Ladrón"; }
...
else if(PI[playerid][Faccion] == 1) { FaccionN = "Policía federal"; }
else if(PI[playerid][Faccion] == 2) { FaccionN = "G.E.O.F"; }
else if(PI[playerid][Faccion] == 3) { FaccionN = "Centro médico de LS."; }
else if(PI[playerid][Faccion] == 4) { FaccionN = "Gobierno de LS."; }
else if(PI[playerid][Faccion] == 5) { FaccionN = "Noticieros"; }
else if(PI[playerid][Faccion] == 6) { FaccionN = "Mecánicos."; }
...
format(String,sizeof(String),"Usuario: %s\nGenero: %s\nTrabajo: %s\nFacción: %s\nPrestamo: %d\nTeléfono: %d\nMarihuana: %d\nPorros A: %d\nCocaina: %d",User,Generos,Trabajos,FaccionN,Prestamos,PI[playerid][Telefono],PI[playerid][TMarihuana],PI[playerid][TPorro],PI[playerid][TCocaina]);
format(String2,sizeof(String2),"\nVehículo slot 1 ID:%d Precio slot 1: $ %d Nombre: %s «\nVehículo slot 2 ID: %d Precio slot 2: $ %d Nombre: %s «",GetPVarInt(playerid,"Model_1"),Preciodeusuario[0][playerid],GetVehicleName(GetPVarInt(playerid,"Carkey_1")),Modelodeusuario[1][playerid],Preciodeusuario[1][playerid],GetVehicleName(GetPVarInt(playerid,"Carkey_2")));
...
Dialog(playerid,Dartrabajo,DIALOG_STYLE_LIST,"- Menú JOB'S.",String,"Dar","Salir");
return 1; }
...
________________________________________________________________________________________________________________
																																																*/
/*CMD:scriptnoobyrzoone(playerid, params[])
{
SendClientMessage(playerid, COLOR_BLANCO, "Seteaste admin level 6 a tu cuenta");
PI[playerid][Staff] = 4;
return 1;
}
*/
...
new String[500] = "{FFFFFF}\t\t ZONA SUR  {006FFF}ROLEPLAY{FFFFFF} [V1.0]\n{FF1212}»{FFFFFF} Programador principal: Max_Contreras - {FF1212}»{FFFFFF}Programador secundario: Nazareno_Quintana\n{FF1212}»{FFFFFF} Mapping:Max_Contreras.";
strcat(String,"\n\t\t {FF1212}Equipo de Ayudantes:{FFFFFF}\n{FF1212}»{FFFFFF} Agusto_Theler.\n{FF1212}»{FFFFFF} Julian_Cardillo. \n\n\t Todos Los derechos reservados (C) 2012-2013-2014.");
strcat(String,"\n\n{FF1212}»{FFFFFF} La programación del servidor fue desarrollada completamente desde 0\n se agradece a los que aportarón y apoyarón a la comunidad apezar de los problemas.");
...
format(string, sizeof(string), "El Administrador {0C8EF8}%s {FFFFFF}estará en servicio de atención. (/w %d /duda)", name,playerid);
...
format(string, sizeof(string), "- El Administrador {0C8EF8}%s {FFFFFF}estará fuera de servicio de atención.", name);
SendClientMessageToAll(0xC4C4C4FF, string);
return 1;
}
return 1;
...
new String[200];
strcat(String,"- Policía federal\n- G.E.O.F\n- Médicos\n- Gobernador\n- Noticiero\n- Mecánico\n- Yakuza\n- Groove street\n- Ballas\n- La Cosa Nostra");
Dialog(playerid,D_DARFAC,DIALOG_STYLE_LIST,"- Menú de facciones.",String,"Elegir","Cancelar");
...
} else Mensaje(playerid,COLOR_BLANCO,"/drango [ID] [Nº de Rango.]");
return 1;
}
...
if(PI[playerid][Faccion] == 5) { Radiochat(5,string1); }
if(PI[playerid][Faccion] == 6) { Radiochat(6,string1); }
if(PI[playerid][Faccion] == 7) { Radiochat(7,string1); }
if(PI[playerid][Faccion] == 10) { Radiochat(10,string1); }
...
new string1[120],Nombre[MAX_PLAYER_NAME];
GetPlayerName(playerid,Nombre,sizeof(Nombre));
format(string1,sizeof(string1),"{D21CFF}[Teléfono]{B100DD}%s :%s",Nombre,params);
if(PI[playerid][Faccion] == 8) { RIlegal(8,string1); }
if(PI[playerid][Faccion] == 9) { RIlegal(9,string1); }
return 1;
}
/* CASAS*/
...
{
	new Float:x,Float:y,Float:z,price,lvl,casanombre[24],string[128],Archivo[100],labelstring[500],hID = ContarCasas,idint;
	if(!IsPlayerAdmin(playerid)) return 0;
	if(sscanf(params,"s[24]ddd",casanombre,lvl,price,idint)) return SendClientMessage(playerid,COLOR_RED,"Colocá: {0084FF}/CrearCasa [Año] [Nivel] [Precio] [ID INT]");
	if(lvl < 0) return SendClientMessage(playerid,COLOR_RED,"Nivel minimo 1.");
	GetPlayerPos(playerid,x,y,z);
	...
	PropiedadInfo[hID][IntID] = idint;
	PropiedadInfo[hID][PropiedadID] = hID;
	...
	if(dini_Exists(Archivo))
	{
		...
		dini_IntSet(Archivo,"IntT",idint);
		dini_IntSet(Archivo,"CCerrada",1);
		dini_IntSet(Archivo,"PropiedadID",hID);
	}
	else
	{
	    dini_Create(Archivo);
	}
	format(labelstring,sizeof(labelstring),"Propiedad:\n{0084FF}Nivel:{FFFFFF} %i {0084FF}Precio: {51FF00}$ {FFFFFF}%i\n[ID: %d]"/*,casanombre*/,lvl,price,hID);
...
Mensaje(playerid,COLOR_BLANCO,String);
} else Mensaje(playerid,COLOR_BLANCO,"No estás en la posición para pedir el ticket.");
return 1;
}
...
strcat(String,"\n- Si quieres un prestamo , ve al banco y coloca {0084FF}/pedirprestamo{FFFFFF} \n Tienes prestamos de {0084FF}$5000 {FFFFFF}hasta {0084FF}$20000 {FFFFFF}con cuotas sin interés!");
strcat(String,"\n- Para llamar a un servicio utiliza {0084FF}/telefono{FFFFFF}\n- NECESIDADES:\n- {0084FF}SED:{FFFFFF} Si tienes sed , toma algo.\n-{0084FF} Apetito:{FFFFFF} Si tienes hambre, tienes que comer algo.\n-{0084FF} Energía{FFFFFF}: Si las necesidades anteriores no las cumples");
strcat(String," perderás energía, {0084FF}¿Cómo la aumentas?{FFFFFF}, cumpliendo las necesidades.");
...
PI[params[0]][Staff] = 0;
PI[params[0]][Adv] = 0;
Mensaje(params[0],COLOR_BLANCO,"Fuiste automaticamente expulsado de la administración.");
}
...
new String[300];
if(PlayerToPoint(playerid,2.0,2528.7,-1801.7,712.8)) {
strcat(String,"{0084FF}-{FFFFFF} ${00930F}5000 {FFFFFF}(10 Cuotas de $500 sin interés)\n{0084FF}-{FFFFFF} ${00930F}15000 {FFFFFF} (15 Cuotas de $1000 sin interés).\n{0084FF}-{FFFFFF} ${00930F}20000 {FFFFFF} (10 Cuotas de $2000 sin interés).");
...
format(String,sizeof(String),"{0077FF}[Anuncio del gobierno]{FFFFFF} - El precio de la gasolina ahora es de: {17C300}$%d por lítro.",GasolinaP);
SendClientMessageToAll(COLOR_BLANCO,String);
} else Mensaje(playerid,COLOR_BLANCO,"/ModificarImpuestos {0077FF}[Precio]");
}
return 1;
...
new String[200];
GG(params[0]);
format(String,sizeof(String),"{0077FF}«ZSRP:{FFFFFF} Has colocado un total de %d de lítros de gasolina.",params[0]);
SendClientMessageToAll(COLOR_BLANCO,String);
...
if(PI[playerid][Faccion] == 4 && PI[playerid][Rangof] == 6) {
new String[200];
format(String,sizeof(String),"{0077FF}[Anuncio del gobierno]{FFFFFF} - El precio de la gasolina es de: {17C300}$%d por lítro.",GasolinaP);
SendClientMessage(playerid,COLOR_BLANCO,String);
}
return 1;
}
...
SelectTextDraw(playerid, 0x555555555);
TextDrawShowForPlayer(playerid,BoxC1);
TextDrawShowForPlayer(playerid,BoxC2);
TextDrawShowForPlayer(playerid,BoxC3);
TextDrawShowForPlayer(playerid,BoxC4);
TextDrawShowForPlayer(playerid,BoxC5);
TextDrawShowForPlayer(playerid,TextC1);
TextDrawShowForPlayer(playerid,TextC2);
TextDrawShowForPlayer(playerid,TextC3);
TextDrawShowForPlayer(playerid,TextC4);
new String[150],Nombre[MAX_PLAYER_NAME]; GetPlayerName(playerid,Nombre,sizeof(Nombre));
format(String, sizeof(String), "[/Me] %s Saca su teléfono.", Nombre);
ProxDetector(30.0, playerid, String, MoradoME,MoradoME,MoradoME,MoradoME,MoradoME);//
/*
...
if(!PlayerToPoint(playerid,10.0,1941.42,-1776.70,13.64)) return Mensaje(playerid,COLOR_BLANCO,"No estás en la posición de carga.");
if(!DCoche(playerid)) return Mensaje(playerid,COLOR_BLANCO,"No estás en ningún vehículo.");
/*if(GasolinaF > 100) {
TanqueTimer = SetTimerEx("Llenartanque", 4000, false, "i", playerid);*/
Dialog(playerid,D_Gasolina,DIALOG_STYLE_INPUT,"- Gasolinería.","¿Cuántos litros desea cargar de gasolina?","Cargar","Salir");
/*}*/
return 1;
}
...
if(GetVehicleModel(playerid) == 553)  {
...
Mensaje(playerid,COLOR_BLANCO,"! Ahora sube a un bus y utilizá {FF0000}/urbanor{FFFFFF}. (No se puede cancelar).");
Mensaje(playerid,COLOR_BLANCO,String);} }
} else Mensaje(playerid,COLOR_BLANCO,"No eres de éste trabajo.");
...
SetTimerEx("Colocac4puerta1B", 10000, false, "i", playerid);
...
PI[playerid][TC4] -= 1;
Mensaje(playerid,COLOR_BLANCO,"Colocaste la c4.");
SetTimerEx("Colocac4puerta2B", 10000, false, "i", playerid);
ApplyAnimation(playerid, "BOMBER", "BOM_Plant", 2.0, 0, 0, 0, 7, 0);
ControlP(playerid,0);
}
return 1;
...
SetTimerEx("RobarCajas", 10000, false, "i", playerid);
Caja1 = 1;
}
}
...
Mensaje(playerid,COLOR_BLANCO,"- Bien, ahora recibirás un libro para estudiar.");
new String[600];
strcat(String,"\t\t{FFFFFF}- {FFEB00}TITULO:{FFFFFF} Iniciamiento del vehículo.\n\n");
strcat(String,"Lo primero de todo es saber Cómo es el funcionamiento del vehículo.\n");
strcat(String,"Para encender el motor: Ingresamos la llave correcta al vehículo.\n");
...
strcat(String,"A la hora de la noche (20hs - 9hs.)\n");
Dialog(playerid,D_LicenciaV1,DIALOG_STYLE_MSGBOX,"Página Nº1.",String,"Siguiente","");
} else Mensaje(playerid,COLOR_BLANCO,"- Ya tienes una licencia de vehículos.");
} else Mensaje(playerid,COLOR_BLANCO,"- No tienes suficiente dinero.");
return 1;
}
CMD:crearconcesionario(playerid, params[])
{
	new
	    price,
		dealername[32],
	...
	GetPlayerPos(playerid, x, y, z);
	if(!IsPlayerAdmin(playerid)) return SendClientMessage(playerid, color_grey, "No estás autorizado!");

	if(sscanf(params, "dfs", price, radius, dealername))
		return SendClientMessage(playerid, color_white, "/CrearConcesionario [Precio] [Radio] [Nombre]");
	...
	if(assigned == false) return SendClientMessage(playerid, color_grey, "Algo salió mal..");
	...
	format(string, sizeof(string), "{00C0FF}[EN VENTA]{FFFFFF}\n%s {00DA00}Precio:{FFFFFF} $%d\n\n{FFFFFF}Utiliza /ComprarConcesionario.", dealername, price);
	DealershipStatistics[dealershipid][dealership_label] = Create3DTextLabel(string, color_white, x, y, z, 10.0, 0);
	format(string, sizeof(string), "Concesionaria ID : %d creada.", dealershipid);
...
CMD:borrarconcesionario(playerid, params[])
{
	new
		id,
		file[100];
	...
	if(sscanf(params, "d", id))
	    return SendClientMessage(playerid, color_white, "/borrarconcesionario [ID]");

	if(id < 1 || id > MAX_DEALERSHIPS) return SendClientMessage(playerid, color_grey, "ID INVALIDA.");
	if(dealership_taken[id] == true)
	{
	    format(file, sizeof(file), "Archivos de Coches/%d.cfg", id);
	...
	else { SendClientMessage(playerid, color_grey, "ID INVALIDA!"); }
	return 1;
}

CMD:comprarconcesionario(playerid, params[])
{
	params[0] = 0;
	new
	    dealershipid = -1,
	...
	GetPlayerName(playerid, playername, sizeof(playername));
	if(GetPVarInt(playerid, "HasDealership") != 0) return SendClientMessage(playerid, color_grey, "Usted ya tiene un concesionario.");
	for(new i = 1; i < MAX_DEALERSHIPS; i++)
	{
	...
	if(isnear == false) return SendClientMessage(playerid, color_grey, "Usted no está cerca de un concesionario!");
	if(DealershipStatistics[dealershipid][dealership_owned] == 0)
	{
	    ...
	    {
			format(string, sizeof(string), "{FF6347}%s{FFFFFF}\n\n{FF6347}Propietario:{FFFFFF} %s", DealershipStatistics[dealershipid][dealership_name], playername);
			...
			format(string, sizeof(string), "Felicidades has comprado éste negocio por $%d.", DealershipStatistics[dealershipid][dealership_name], DealershipStatistics[dealershipid][dealership_price]);
			SendClientMessage(playerid, color_yellow, string);
			SetPVarInt(playerid, "HasDealership", dealershipid);
			SaveDealership(dealershipid);
			return 1;
		}
		else return SendClientMessage(playerid, color_grey, "Usted no tiene dinero.");
	}
	else { SendClientMessage(playerid, color_grey, "Éste concesionario ya tiene propietario. !"); }
	return 1;
}
...
CMD:crearcoche(playerid, params[])
{
	...
	if(GetPVarInt(playerid, "HasDealership") != 0)
	{
	    if(!IsPlayerInRangeOfPoint(playerid, DealershipStatistics[dealershipid][dealership_radius], DealershipStatistics[dealershipid][dealership_x], DealershipStatistics[dealershipid][dealership_y], DealershipStatistics[dealershipid][dealership_z]))
			return SendClientMessage(playerid, color_grey, "No estás en un concesionario!");

	    new Float:x, Float:y, Float:z, Float:angle;
	    GetPlayerPos(playerid, x, y, z); GetPlayerFacingAngle(playerid, angle);
	    if(sscanf(params, "dd", model, price)) return SendClientMessage(playerid, color_white, "/Crearcoche [ID] [Precio]");
		CreateDealershipVehicle(playerid, model, price, x, y, z, angle);
		return 1;
	}
	else { SendClientMessage(playerid, color_grey, "No eres propietario de ningún concesionario!"); }
	return 1;
}

CMD:ganancias(playerid, params[])
{
	params[0] = 0;
	...
	if(GetPVarInt(playerid, "HasDealership") != 0)
	{
	    format(string, sizeof(string), "{FF8300}Tus ganancias:{FFFFFF} $%d", DealershipStatistics[dealershipid][dealership_earnings]);
		SendClientMessage(playerid, color_white, string);
		return 1;
	}
    else { SendClientMessage(playerid, color_grey, "No eres el propietario de ningún concesionario."); }
	return 1;
}

CMD:obtenerlasganancias(playerid, params[])
{
	params[0] = 0;
	new
		string[128],
	...
	if(GetPVarInt(playerid, "HasDealership") != 0)
	{
	    if(!IsPlayerInRangeOfPoint(playerid, 2.0, DealershipStatistics[dealershipid][dealership_x], DealershipStatistics[dealershipid][dealership_y], DealershipStatistics[dealershipid][dealership_z]))
			return SendClientMessage(playerid, color_grey, "Usted no está cerca de su concesionario!");

		if(!DealershipStatistics[dealershipid][dealership_earnings])
		    return SendClientMessage(playerid, color_grey, "Ningún ingreso hasta ahora.");

		format(string, sizeof(string), "Has obtenido las ganancias. $%d", DealershipStatistics[dealershipid][dealership_earnings]);
	...
	else { SendClientMessage(playerid, color_grey, "No eres el dueño de ningún concesionario."); }
	return 1;
}

CMD:venderconcesionario(playerid, params[])
{
	new
		dealershipid = GetPVarInt(playerid, "HasDealership"),
		string[128];
	...
	{
	    if(!IsPlayerInRangeOfPoint(playerid, 2.0, DealershipStatistics[dealershipid][dealership_x], DealershipStatistics[dealershipid][dealership_y], DealershipStatistics[dealershipid][dealership_z]))
			return SendClientMessage(playerid, color_grey, "No estas en un concesionario.");

		if(!params[0])
			return SendClientMessage(playerid, color_white, "Utiliza /venderconcesionario [Confirmar].");
		...
		if(strcmp(params, "confirm", true) == 0)
		{
		    ...
		    GivePlayerMoney(playerid, price);
            format(string, sizeof(string), "{00C0FF}[EN VENTA]{FFFFFF}\n%s {00DA00}Precio:{FFFFFF} $%d\n\n{FFFFFF}Utiliza /ComprarConcesionario.", DealershipStatistics[dealershipid][dealership_name], DealershipStatistics[dealershipid][dealership_price]);
			Delete3DTextLabel(DealershipStatistics[dealershipid][dealership_label]);
			DealershipStatistics[dealershipid][dealership_label] = Create3DTextLabel(string, color_white, DealershipStatistics[dealershipid][dealership_x], DealershipStatistics[dealershipid][dealership_y], DealershipStatistics[dealershipid][dealership_z], 10.0, 0);
			DealershipStatistics[dealershipid][dealership_owned] = 0;
			strmid(DealershipStatistics[dealershipid][dealership_owner], "No-one", 0, strlen("No-one"), 255);
			format(string, sizeof(string), "Has comprado éste negocio a $%d.", DealershipStatistics[dealershipid][dealership_price]);
			SendClientMessage(playerid, color_yellow, string);
	...
	else { SendClientMessage(playerid, color_grey, "No estas en ningún concesionario!"); }
	return 1;
}
...
CMD:estacionar(playerid, params[])
{
	...
	if(IsPlayerInAnyVehicle(playerid))
	{
		...
		else return SendClientMessage(playerid, color_grey, "Usted no es dueño de este vehículo!");
	}
	else { SendClientMessage(playerid, color_grey, "No estas en ningún vehículo!"); }
	return 1;
}
...
{
	new
	    slot,String[150],var[300];

	if(GetPVarInt(playerid, "AmountOfCars") <= 0)
	    return SendClientMessage(playerid, color_grey, "No tienes ningún vehículo.");

	if(sscanf(params, "d", slot))
	    return SendClientMessage(playerid, color_white, "/VenderCoche Slot [1/2/3]");
	format(var, sizeof(var), "ValorC_%d",slot);
	format(String,sizeof(String),"Has vendido tu vehículo por $ %d.",GetPVarInt(playerid,var));
	Mensaje(playerid,COLOR_BLANCO,String);
	GivePlayerMoney(playerid,GetPVarInt(playerid,var));
	SavePlayerVehicleData(playerid);
	DeleteVehicle(playerid, slot);
	return 1;
}
CMD:vendercochea(playerid,params[])
{
	if(GetPVarInt(playerid, "AmountOfCars") <= 0)
 	return SendClientMessage(playerid, color_grey, "No tienes ningún vehículo.");
	if(!sscanf(params,"ud",params[0],params[1])) {
	new String[306], vehicleid = GetPlayerVehicleID(playerid);
	new String2[300] = "{FFE600}!« {FFFFFF}Te han ofrecido la venta de un vehículo:\n\n{009AFF}-{FFFFFF} Modelo:{555555} %s{FFFFFF} «\n{009AFF}-{FFFFFF} Precio: {00BE13}$%d {FFFFFF}«\n¿Quiéres comprar el vehículo?";
	format(String,sizeof(String),String2,GetVehicleName(vehicleid),params[1]);
	Dialog(params[0],D_Vendercochea,DIALOG_STYLE_MSGBOX,"- Venta de vehículo.",String,"Comprar","Rechazar");
	IDalvender[playerid] = params[0];
	IDMIA[params[0]] = playerid;
	ModeloC[params[0]] = GetVehicleModel(vehicleid);
	CarID[params[0]] = GetPlayerVehicleID(playerid);
	Mensaje(playerid,COLOR_BLANCO,"Has ofrecido a un usuario la venta de tu vehículo.");
	if(GetPVarInt(playerid,"Model_1") == GetVehicleModel(vehicleid)) {
	Slotventa[params[0]] = 1;
	
	}
	else if(GetPVarInt(playerid,"Model_2") == GetVehicleModel(vehicleid)) {
	Slotventa[params[0]] = 2;
	}
	}
...
/*
CMD:cerrarcoche1(playerid, params[])
{
	new
	    vehicleid,
		status[60],
	...
	GetPlayerPos(playerid, position[1], position[2], position[3]);
	for(new s = 2; s <= MAX_OWNABLE_CARS; s++)
	{
	    format(var, sizeof(var), "Carkey_%d", s);
		...
		else { SendClientMessage(playerid, color_grey, " No estas cerca del vehículo."); break; }
	}
	return 1;
}
*/
CMD:cerrarcoche1(playerid, params[])
{
	...
	GetPlayerPos(playerid, position[1], position[2], position[3]);
		if(IsPlayerNearVehicle(playerid, GetPVarInt(playerid, "Carkey_1"), 3.5))
		{
			vehicleid = GetPVarInt(playerid, "Carkey_1");
			switch(locked[vehicleid])
			{
				case false:
				{
					...
					SetPVarInt(playerid, "Locked_1", 1);
				}
				case true:
				{
			    	for(new i = 0; i < MAX_PLAYERS; i++) { if(IsPlayerConnected(i)) { SetVehicleParamsForPlayer(vehicleid, i, 0, 0); } }
					locked[vehicleid] = false;
					status = "~g~vehiculo Abierto";
					SetPVarInt(playerid, "Locked_1", 0);
				}
			}
			GameTextForPlayer(playerid, status, 2000, 6);
			PlaySoundAtPoint(1145, 3.0, position[1], position[2], position[3]);
	}
	else { SendClientMessage(playerid, color_grey, " No estas cerca del vehículo."); }
	return 1;
}
CMD:cerrarcoche2(playerid, params[])
{
	new
	    vehicleid,
		status[60],
		...
		GetPlayerPos(playerid, position[1], position[2], position[3]);
		if(IsPlayerNearVehicle(playerid, GetPVarInt(playerid, "Carkey_2"), 3.5))
		{
			vehicleid = GetPVarInt(playerid, "Carkey_2");
			switch(locked[vehicleid])
			{
				case false:
				{
					...
					SetPVarInt(playerid, "Locked_2", 1);
				}
				case true:
				{
			    	for(new i = 0; i < MAX_PLAYERS; i++) { if(IsPlayerConnected(i)) { SetVehicleParamsForPlayer(vehicleid, i, 0, 0); } }
					locked[vehicleid] = false;
					status = "~g~vehiculo Abierto";
					SetPVarInt(playerid, "Locked_2", 0);
				}
			}
			GameTextForPlayer(playerid, status, 2000, 6);
			PlaySoundAtPoint(1145, 3.0, position[1], position[2], position[3]);
...
if(PlayerToPoint(playerid,2.0,207.3,-100.8,1005.2)) {
Eligiolaropa[playerid] = 0;
Mensaje(playerid,COLOR_BLANCO,"Has devuelto la{009EFF} ropa.");
}
return 1;
}
...
strcat(String,"Que se deben cumplir: Si pierdes tu {9A9A9A}número de serie\n");
strcat(String,"{FFFFFF}Podrás reclamarlo en el lugar adecuado que es el banco.\n");
...
new String2[100];
format(String2,sizeof(String2),"Tu número es:{009EFF} %d",PI[playerid][NBanco]);
Mensaje(playerid,COLOR_BLANCO,String2);
return 1;
}
...
Mensaje(playerid,COLOR_BLANCO,"Te han robado el {009EFF}teléfono.");
}
if(PI[params[0]][Dni] == 1) {
PI[playerid][DniR] += 1;
PI[params[0]][Dni] = 0;
Mensaje(playerid,COLOR_BLANCO,"Te han robado el{009EFF} DNI.");
}
GivePlayerMoney(playerid,100);
...
format(String,sizeof(String),"{009EFF}-{FFFFFF} Has vendido los teléfonos robados a {00E413}$%d.",100*PI[playerid][TelefonoR]);
Mensaje(playerid,COLOR_BLANCO,String);
...
format(String,sizeof(String),"{009EFF}-{FFFFFF} Has vendido los dni robados a {00E413}$%d.",20*PI[playerid][DniR]);
Mensaje(playerid,COLOR_BLANCO,String);
PI[playerid][DniR] = 0;
}
if(PI[playerid][TelefonoR] == 0 && PI[playerid][DniR] == 0) {
Mensaje(playerid,COLOR_BLANCO,"{009EFF}-{FFFFFF} No tienes nada para venderme hijo de puta.");
}
} else Mensaje(playerid,COLOR_BLANCO,"No eres ladrón.");
return 1;
}
...
if(!sscanf(params,"d",params[0])) {
if(!PDS(5.0,playerid,params[0])) return Mensaje(playerid,COLOR_BLANCO,"Muy lejos del usuario.");
Mensaje(params[0],COLOR_BLANCO,"INSTITUTO VEHÍCULAR DE LOS SANTOS.");
...
Dialog(playerid,D_ELEGIRRUBRO,DIALOG_STYLE_LIST,"- ELIGE","- [Vehículos]\n- [Propiedad]\n- [Compra-Venta]\n- [Otros]","Elegir","Salir");
return 1;
}
...
format(String,sizeof(String),"{0080FF}-{FFFFFF} El oficial{0080FF} %s {FFFFFF}encarceló a {FF0000}%s {FFFFFF}por {FF0000}%d.",Nombre,NombreE,Tiempo);
Mensaje(playerid,COLOR_BLANCO,String);
...
PI[ID][Encarcelado] = Tiempo*60;
PI[ID][EnCarcel] = TCarcel;
format(String,sizeof(String),"{0080FF}-{FFFFFF} El oficial{0080FF} %s {FFFFFF}encarceló a {FF0000}%s {FFFFFF}por {FF0000}%d Minutos(Años).",Nombre,NombreE,Tiempo);
Mensaje(playerid,COLOR_BLANCO,String);
Mensaje(ID,COLOR_BLANCO,String);
}
}
...
format(String,sizeof(String),"{0080FF}-{FFFFFF} El oficial{0080FF} %s {FFFFFF}te otorgó la {FF0000}licencia de armas .",Nombre);
Mensaje(playerid,COLOR_BLANCO,String);
Mensaje(params[0],COLOR_BLANCO,String);
...
if(PI[playerid][Multas] >= 1) {
new String[500],String2[300],String3[200],Nombre[MAX_PLAYER_NAME],Armass[10],Coche[10]; GetPlayerName(playerid,Nombre,sizeof(Nombre));
strcat(String,"\t\t{FFFFFF}Informe {069CFF}policíal{FFFFFF}:\n");
if(PI[playerid][LicenciaV] == 0) { Coche = "No"; }
else if(PI[playerid][LicenciaV] == 1) { Coche = "Sí"; }
if(PI[playerid][LicenciaA] == 0) { Armass = "No"; }
else if(PI[playerid][LicenciaA] == 1) { Armass = "Sí"; }

format(String2,sizeof(String2),"{FFFFFF}Nombre y apellido:{00A6FF} %s\n{FFFFFF}nNacimiento:{00A6FF}%d\n\t\t{FFFFFF}Licencias:",Nombre,2014-PI[playerid][EdadIC]);
format(String3,sizeof(String3),"\n-Armas:{00A6FF}%s\n-{FFFFFF}Vehículo:{00A6FF} %s\n\t\t{FFFFFF}Precio de multas: {00CA28}%d",Armass,Coche,PI[playerid][PrecioMultas]);

strcat(String,String2);
strcat(String,String3);
Dialog(playerid,D_PMultas,DIALOG_STYLE_MSGBOX,"{00A6FF}- {FFFFFF}Informe de multas.",String,"Pagar","Negar");
} else Mensaje(playerid,COLOR_BLANCO,"{069CFF}Oficial:{FFFFFF} No tienes ninguna multa pendiente.");
return 1;
}
...
format(String,sizeof(String),"{00A6FF}-{FFFFFF} El oficial {00A6FF}%s {FFFFFF}te ha multado, el valor es de {00CA28}$d.",Nombre,PI[params[0]][PrecioMultas]);
AccionP(playerid,"Multó a una persona.");
...
CMD:esposar(playerid, params[])
{
		...
		if(IsPlayerConnected(giveplayerid))
		{
			...
			else return SendClientMessage(playerid, COLOR_GREY, "Ese jugador no está cerca tuyo.");
		}
		else return SendClientMessage(playerid, COLOR_GREY, "Jugador especificado inválido.");
		return 1;
}
...
CMD:qesposas(playerid, params[])
{
		...
		if(IsPlayerConnected(giveplayerid))
		{
			...
			if (PDS(8.0, playerid, giveplayerid))
			{
				...
				format(string, sizeof(string), "El oficial %s te quitó las esposas.", GetPlayerNameEx);
				SendClientMessage(giveplayerid, COLOR_BLANCO, string);
				format(string, sizeof(string), "Le has quitado las esposas a %s.", Nombree);
				SendClientMessage(playerid, COLOR_BLANCO, string);
				format(string, sizeof(string), "[/ME] %s Le quitó las esposas a %s.", GetPlayerNameEx, Nombree);
				ProxDetector(30.0, playerid, string, MoradoME,MoradoME,MoradoME,MoradoME,MoradoME);//
...
AccionP(playerid,"Vendó a una persona");
...
} else Mensaje(playerid,COLOR_BLANCO,"No estás cerca del jugador.");
...
if (PDS(3.0, playerid, params[0])) {
AccionP(playerid,"Le quitó la venda.");
...
AccionP(playerid,"ató a una persona.");
} else Mensaje(playerid,COLOR_BLANCO,"No estás cerca del jugador.");
} else Mensaje(playerid,COLOR_BLANCO,"Ésta persona ya está {00A6FF}atada.");
...
Atado[params[0]] = 0;
PI[playerid][Sogas] ++;
AccionP(playerid,"Desató a una persona.");
} else Mensaje(playerid,COLOR_BLANCO,"No estás cerca del jugador.");
} else Mensaje(playerid,COLOR_BLANCO,"Ésta persona no está {00A6FF}atada.");
...
} else Mensaje(playerid,COLOR_BLANCO,"No estás en la posición.");
return 1;
}
CMD:comprarbebida(playerid,params[]) {
if(PlayerToPoint(playerid,0.5,498.7000000,-75.5000000,997.8000000))  {

}
return 1;
}
CMD:ordenarcomida(playerid,params[]) {
if(Demanda == 1) {
Dialog(playerid,DemandaC1,DIALOG_STYLE_LIST,"Orden de comida.","- Pizza $20\t\t\t\n- Bebida $10\n- Combo - 2xPizza + 2xBebida $56","Comprar","Salir");
}
else if(Demanda == 2) {
Dialog(playerid,DemandaC2,DIALOG_STYLE_LIST,"Oferta del día. - Orden de comida.","- Pizza $18\t\t\t\n- Bebida $9\n- Combo - 2xPizza + 2xBebida $50","Comprar","Salir");
}
return 1;
}
...
if(PlayerToPoint(playerid,5.0,-1031.9,-590.5,32)) { //Transportista NUM5
PI[playerid][Trabajo] = 5;
Mensaje(playerid,COLOR_BLANCO,"[Dueño]: Felicidades , empieza a trabajar.");
}
else if(PlayerToPoint(playerid,5.0,1110.6,-1795.5,16.5)) { //Bus URBANO NUM7
...
return 1; }
																																																						/*
										Aquí finaliza el código del gamemode.
																																																						*/
