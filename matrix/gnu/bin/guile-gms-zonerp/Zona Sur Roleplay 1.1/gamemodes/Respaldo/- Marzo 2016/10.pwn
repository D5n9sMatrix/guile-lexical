...
18-Entrada y salida con rol. -Con letra Y
...
/*
	...
	Tit3 = TextDrawCreate(254.000000, 327.000000, "~w~Roleplay - Develop");
...
public OnPlayerEnterVehicle(playerid, vehicleid, ispassenger)
{
	...
	if(!ispassenger) {
    if(Motorencendido[vehicleid] == 0) { Mensaje(playerid,Gris,"Atención:{FFFFFF} El motor del coche está apagado.Coloca{555555} /PrenderMotor"); }
...
//2189.57,-1978.87,13.55 - 1956.0996000,-2106.7998000,12.6000000
	return 1;
}
...
new String[128],String2[50];
if(PI[playerid][VehiculoID2] != 999) {
format(String2,sizeof(String2),"1- Vehículo ID: %i - Modelo: %d [ %s ]",PI[playerid][VehiculoID1],GetVehicleModel(PI[playerid][VehiculoID1]+MAX_VEHICULOS+1),NombreCoches[GetVehicleModel(PI[playerid][VehiculoID1]+MAX_VEHICULOS+1)-400]);

}
else if(PI[playerid][VehiculoID1] != 999) {
format(String2,sizeof(String2),"1- Vehículo ID: %i - Modelo: %d [ %s ]\n2- Vehículo ID: %i - Modelo: %d [ %s ]",PI[playerid][VehiculoID1],GetVehicleModel(PI[playerid][VehiculoID1]+MAX_VEHICULOS+1),NombreCoches[GetVehicleModel(PI[playerid][VehiculoID1]+MAX_VEHICULOS+1)-400],PI[playerid][VehiculoID2],GetVehicleModel(PI[playerid][VehiculoID2]+MAX_VEHICULOS+1),NombreCoches[GetVehicleModel(PI[playerid][VehiculoID2]+MAX_VEHICULOS+1)-400]);

}
strcat(String,String2);
JugadorTransferencia[playerid] = strval(inputtext);
Dialog(playerid,D_TransferenciaVeh2,DIALOG_STYLE_INPUT,"- Selecciona el vehículo que quieres transferir.",String,"Seleccinoar","Salir");
}
}
...
CI[VehiculosCreados+MAX_VEHICULOS+1][CID]=AddStaticVehicleEx (ConteoC[playerid][EligioItemC[playerid]-1], 532.17,-1288.40,17.24, 82.2873, EligioColorC[playerid], -1, 90000);
...
Mensaje(playerid,COLOR_BLANCO,"- Comandos: {009EFF}/CargarCamion .");
}
...
Mensaje(playerid,COLOR_BLANCO,"- Comandos: {009EFF}/empezaratrabajar ");
}
}
}
}
...
new Trabajos[30],Generos[20],String[700],User[MAX_PLAYER_NAME],FaccionN[35],Prestamos[40];
GetPlayerName(playerid,User,sizeof(User));
...
format(String,sizeof(String),"Usuario: %s\nGenero: %s\nTrabajo: %s\nFacción: %s\nPrestamo: %d\nTeléfono: %d\nMarihuana: %d\nPorros A: %d\nCocaina: %d\nVehículo ID1: %d\nVehículo ID2: %d",User,Generos,Trabajos,FaccionN,Prestamos,PI[playerid][Telefono],PI[playerid][TMarihuana],PI[playerid][TPorro],PI[playerid][TCocaina],PI[playerid][VehiculoID1]+MAX_VEHICULOS+1,PI[playerid][VehiculoID2]+MAX_VEHICULOS+1);
...
CMD:pedirautorizacion(playerid,params[]) {
    if(PI[playerid][Trabajo] == 3) {
	if(GetVehicleModel(playerid) == 553)  {
	
	Mensaje(playerid,COLOR_BLANCO,"[!]- Para empezar pide permiso para despegar. /PedirAutorizacion.");
...
CMD:cerrarcoche(playerid,params[]) {
	new Float:CXX,Float:CYY,Float:CZZ,String[128];
	print("a");
	for(new i=0;i<VehiculosCreados;i++) {
...
CMD:estacionar(playerid,params[]) {
	...
	format(String,sizeof(String),"Estacionaste tu coche en las coordenadas: X: %f Y: %f Z: %f",CXX,CYY,CZZ);
	SendClientMessage(playerid,-1,String);
	}
	...
	GetVehiclePos(GetPlayerVehicleID(playerid),CXX,CYY,CZZ);
	dini_FloatSet(String,"CX",CXX); dini_FloatSet(String,"CY",CYY); dini_FloatSet(String,"CZ",CZZ);
	format(String,sizeof(String),"Estacionaste tu coche en las coordenadas: X: %f Y: %f Z: %f - Mismo propietario",CXX,CYY,CZZ);
	SendClientMessage(playerid,-1,String);
	}
