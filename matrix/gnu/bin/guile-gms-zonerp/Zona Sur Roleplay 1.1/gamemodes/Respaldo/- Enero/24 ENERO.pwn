...
//Echo	Sistema de inventario.
		...
		Sistema de anti abusos admin's, 3 advertencia será removido el admin directamente.
...
stock CargarNegocios()
{
	...
	for(new i = 0; i < MAX_NEGOCIOS; i++)
	{
		...
		if(dini_Exists(file))
		{
			...
			PropiedadInfo[i][CasaVirtual] = dini_Int(file,"VirtualWorld");*/
			NProp = dini_Get(file,"DuNombre");
			format(NegocioInfo[i][DuNombre],24,"%s",NProp);
			NegocioEntrada[i] = CreatePickup(1273,1,NegocioInfo[i][NX],NegocioInfo[i][NY],NegocioInfo[i][NZ],0);
			new labelstring[500];
			switch(NegocioInfo[i][Comprada])
			{
				case 0:{format(labelstring,sizeof(labelstring),"{555555}Negocio:{FFFFFF} %s \n{555555}Nivel:{FFFFFF} %i \n{555555}Precio: {51FF00}$ {FFFFFF}%i",NegocioInfo[i][NLema],NegocioInfo[i][Nivel],NegocioInfo[i][Precio]);}
				case 1:{format(labelstring,sizeof(labelstring),"{555555}Negocio:{FFFFFF} %s\n{555555}Estado: {FFFFFF}No disponible -Vendida-\n {555555}Nivel: {FFFFFF}%i\n{555555}Precio:{51FF00}${FFFFFF}%i\n{555555}Dueño: {FFFFFF}%s",NegocioInfo[i][NLema],NegocioInfo[i][Nivel],NegocioInfo[i][Precio],NegocioInfo[i][DuNombre]);}
			}
...
if(PI[i][TNegocio] == 1) {
if(PI[i][InvirtioN] >= 1) {
format(String,sizeof(String),"Invertiste: %d Ganancia: 200$",PI[i][InvirtioN]);
SendClientMessage(i,COLOR_BLANCO,String);
GivePlayerMoney(i, PI[i][InvirtioN]+200);
PI[i][InvirtioN] = 0;
}
}
...
if(listitem == 0) {
Dialog(playerid,D_InvertirN,DIALOG_STYLE_INPUT,"Invirtiendo en el negocio.","¿Cuánto quieres invertir en el negocio?","Invertir","Salir");
}
if(listitem == 1) {
...
{
	new Float:x,Float:y,Float:z,price,lvl,NegoLema[24],string[128],Archivo[100],labelstring[500],NID = ContarNegocios;
	if(!IsPlayerAdmin(playerid)) return 0;
	if(sscanf(params,"s[24]dd",NegoLema,lvl,price)) return SendClientMessage(playerid,COLOR_RED,"Colocá: {555555}/CrearNegocios [Lema] [Nivel] [Precio]");
	...
	format(string,sizeof(string),"{555555}Negocio:{FFFFFF} %s  \n{555555}Nivel:{FFFFFF} %d \n{555555}Precio: \n{51FF00}$ {FFFFFF}%d",NegocioInfo[NID][NLema],NegocioInfo[NID][Nivel],NegocioInfo[NID][Precio]);
    SendClientMessage(playerid,COLOR_RED,"{FFFFFF}Sistema de Negocios:{555555} Propiedad Creada.");
    SendClientMessage(playerid,COLOR_LIME,string);
    NegocioEntrada[NID] = CreatePickup(1273,1,x,y,z,GetPlayerVirtualWorld(playerid));
	...
	format(labelstring,sizeof(labelstring),"{555555}Negocio:{FFFFFF} %s \n{555555}Nivel:{FFFFFF} %d \n{555555}Precio: {51FF00}$ {FFFFFF}%d",NegoLema,lvl,price);
...
CMD:crearcasa(playerid,params[])
{
	...
	format(string,sizeof(string),"{0084FF}Nombre:{FFFFFF} %s {0084FF}Nivel:{FFFFFF} %d {0084FF}Precio: {51FF00}$ {FFFFFF}%d",PropiedadInfo[hID][CaNombre],PropiedadInfo[hID][Nivel],PropiedadInfo[hID][Precio]);
	...
	format(labelstring,sizeof(labelstring),"{0084FF}Nombre:{FFFFFF} %s {0084FF}Nivel:{FFFFFF} %d {0084FF}Precio: {51FF00}$ {FFFFFF}%d",casanombre,lvl,price);
...
PI[playerid][Dni] = 1;*/
Mensaje(playerid,COLOR_BLANCO,"Señora: Espere por favor...(Espera 2 Minutos para recibir tu dni.)");
ControlP(playerid,0);
...
new User[30],Razon[120],NombreA[MAX_PLAYER_NAME],NombreS[MAX_PLAYER_NAME];
if(!sscanf(params,"uu",User,Razon)) {
GetPlayerName(User,NombreA,sizeof(NombreA));
