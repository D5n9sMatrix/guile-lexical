...
//Listo		Sistema de vendar.
			...
			Sistema de bomberos.
			Vender coche - Vendará coche.
			Sistema de Hambre.
			Sistema de liquido.
			Vender vehículo al usuario.
			Nuevos mappeos.
...
new String[200];
PI[i][SubNivel]++;
Mensaje(i,COLOR_BLANCO, "{FFFFFF}»{00B7FF}Día de paga{FFFFFF}«");
Mensaje(i,COLOR_BLANCO, "{CE0000}-{FFFFFF} Ganancia:{00D20A} 300$");
...
public OnPlayerStateChange(playerid, newstate, oldstate)
{
	...
	{
	format(dialog_string, sizeof(dialog_string), "¿Quiéres comprar éste vehículo %s con un precio de $%d?", GetVehicleName(vehicleid), VehicleStatistics[vehicleid][vehicle_price]);
...
CMD:vendercochea(playerid,params[])
{
	if(GetPVarInt(playerid, "AmountOfCars") <= 0)
 	return SendClientMessage(playerid, color_grey, "No tienes ningún vehículo.");
	if(!sscanf(params,"ud",params[0],params[1])) {
	new String[150], vehicleid = GetPlayerVehicleID(playerid);
	format(String,sizeof(String),"- ¿Quieres comprar el vehículo que te han ofrecido por $ %d?",params[1]);
...
} else Mensaje(playerid,COLOR_BLANCO,"/QuitarVendas {00A6FF}[ID]");
return 1;
}
