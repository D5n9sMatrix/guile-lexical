...
stock ModifyVehicles(playerid)
{
	...
	for(new s = 1; s <= MAX_OWNABLE_CARS; s++)
	{
		...
		if(vehicleid > 0)
		{
  			if(GetPVarInt(playerid,"Model_1") == GetVehicleModel(playerid))
	    	{
...
/*
    ...
    txtAnimHelper =
	TextDrawCreate(610.0, 400.0,"~r~ Pulsa ~b~espacio ~r~para detener la animacion");
    TextDrawUseBox(txtAnimHelper, 0);
	TextDrawFont(txtAnimHelper, 3);
...
format(String,sizeof(String),"{FF0000}<!>{FFFFFF} El administrador {0084FF}%s{FFFFFF} dio una facción a {0084FF}%s{FFFFFF}. Faccion ID: {FF8900}%d",Nombre,Nombre2,Facciondar[playerid]);
...
format(mensaje, sizeof(mensaje), "[ACCIÓN] %s [%s]", NombreJugador(playerid), params);
...
CMD:fondocolocar(playerid,params[]) {
...
if(GetVehicleModel(playerid) == 431) {
...
{
format(String,sizeof(String), "{09F36F}[RADIO ILEGAL] {FFFFFF}%s: {FFFFFF}%s",pnamep,params);
...
command(sentarse, playerid, params[])
	...
	command(ped, playerid, params[]){
	    new anim;
        if(sscanf(params, "d", anim)) return SendClientMessage(playerid,COLOR_WHITE,"USA: /ped [1-26]");
        if(!IsPlayerInAnyVehicle(playerid)) return SendClientMessage(playerid, COLOR_GREY, "No puedes hacer esto ahora !");
 		switch(anim){
    ...
    command(sexo, playerid, params[]){
        ...
        if(sscanf(params, "d", sexstyle)) return SendClientMessage(playerid, COLOR_WHITE, "USA: /sexo [1-8]");
 		switch(sexstyle){
			...
			default: return SendClientMessage(playerid,0xFF0000FF,"USA: /fuckme [style 1-8]");
		}
 	  	return 1;
	}
...
format(String,sizeof(String),"[/ME] %s le roban.",Nombre);
