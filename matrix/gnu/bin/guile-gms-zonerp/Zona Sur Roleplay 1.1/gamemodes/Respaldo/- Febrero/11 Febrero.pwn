...
//__Concesionario SISTEMA.____/
#define MAX_COCHESC 1200
enum CocheC {
Propietario[24],
Float:CX,
Float:CY,
...
new ContarCoches;
new InfoCoche[MAX_COCHESC][CocheC];
#define TotalCoches MAX_COCHESGM+MAX_COCHESC                                                                                                                              /*
...
forward CargarCochesC();
public CargarCochesC() {
for(new c = 0; c < MAX_COCHESC; c++)
{   
	new CID = ContarCoches,Archivo[80];

	format(Archivo,sizeof(Archivo),"Archivos de Coches/%i.ini",CID);
	if(!dini_Exists(Archivo)) continue;
	if(dini_Exists(Archivo))
	{
		AddStaticVehicleEx(dini_Int(Archivo,"ModeloID"),dini_Float(Archivo,"X"),dini_Float(Archivo,"Y"),dini_Float(Archivo,"Z"),0.0000000,54,65,600000*6000000); //Nevada 01
		new Coche[500];
		
		new CocheID = GetPlayerVehicleID(c);
		InfoCoche[c][Comprado] = 1;
		Coche = dini_Get(Archivo,"Propietario");
		format(InfoCoche[CocheID][Propietario],24,"%s",Coche);
...
/*
	...
	/* RENTA VEHÍCULOS*/
	RentaV[0] =	AddStaticVehicleEx(410,1561.8000500,-2308.8999000,13.3000000,90.0000000,255,255,15); //Manana 16
	RentaV[1] =	AddStaticVehicleEx(410,1561.8000500,-2312.3000500,13.3000000,90.0000000,255,255,15); //Manana17
	RentaV[2] =	AddStaticVehicleEx(410,1561.9000200,-2315.3999000,13.3000000,90.0000000,255,255,15); //Manana18
	RentaV[3] =	AddStaticVehicleEx(410,1562.0000000,-2318.5000000,13.3000000,90.0000000,255,255,15); //Manana19
	RentaV[4] =	AddStaticVehicleEx(410,1561.8000500,-2321.6999500,13.3000000,90.0000000,255,255,15); //Manana20
	RentaV[5] =	AddStaticVehicleEx(410,1561.5999800,-2325.6001000,13.3000000,90.0000000,255,255,15); //Manana21
	RentaV[6] =	AddStaticVehicleEx(410,1561.6999500,-2328.8000500,13.3000000,90.0000000,255,255,15); //Manana22
	RentaV[7] =	AddStaticVehicleEx(410,1561.8000500,-2332.1999500,13.3000000,90.0000000,255,255,15); //Manana23
	RentaV[8] =	AddStaticVehicleEx(410,1561.8000500,-2335.3000500,13.3000000,90.0000000,255,255,15); //Manana24
	RentaV[9] =	AddStaticVehicleEx(410,1561.8000500,-2338.6001000,13.3000000,90.0000000,255,255,15); //Manana25
...
new Nombre[MAX_PLAYER_NAME],PropCoche[24],CID = ContarCoches,Archivo[80],Float:x,Float:y,Float:z,Float:UX,Float:UY,Float:UZ;
GetPlayerName(playerid,Nombre,sizeof(Nombre));
new Coche = GetPlayerVehicleID(playerid);
GetPlayerPos(playerid,UX,UY,UZ);
AddStaticVehicleEx(params[0],UX,UY,UZ,0.0000000,54,65,15); //Nevada 01
InfoCoche[Coche][Comprado] = 1;
format(PropCoche,sizeof(PropCoche),"%s",Nombre);
InfoCoche[playerid][Propietario] = PropCoche;
ContarCoches++;
format(Archivo,sizeof(Archivo),"Archivos de Coches/%i.ini",CID);
dini_Create(Archivo);
GetPlayerPos(Coche,x,y,z);
if(dini_Exists(Archivo))
	{
	    dini_IntSet(Archivo,"Comprado",InfoCoche[Coche][Comprado]);
	    dini_Set(Archivo,"Propietario",InfoCoche[playerid][Propietario]);
		dini_FloatSet(Archivo,"X",x);
		dini_FloatSet(Archivo,"Y",y);
		dini_FloatSet(Archivo,"Z",z);
		dini_IntSet(Archivo,"ModeloID",params[0]);
	}
	else
	{
	    dini_Create(Archivo);
}
} else Mensaje(playerid, COLOR_BLANCO,"/Conce [ID]");
return 1;
}
CMD:estacionar(playerid,params[]) {
for(new c = 0; c < MAX_COCHESC; c++)
{
new CID = ContarCoches,Archivo[80],Float:x,Float:y,Float:z;
new Coche = GetPlayerVehicleID(playerid);
format(Archivo,sizeof(Archivo),"Archivos de Coches/%i.ini",GetPlayerVehicleID(playerid)-MAX_COCHESGM-601);
GetPlayerPos(playerid,x,y,z);
dini_FloatSet(Archivo,"X",x);
dini_FloatSet(Archivo,"Y",y);
dini_FloatSet(Archivo,"Z",z);
...
format(String,sizeof(String),"%d",GetPlayerVehicleID(playerid)-MAX_COCHESGM-601);
*/

new String[120];
new Coche = GetPlayerVehicleID(playerid);
format(String,sizeof(String),"¿Es comprado?:%d - %s",InfoCoche[Coche][Comprado],InfoCoche[Coche][Propietario]);
Mensaje(playerid,COLOR_BLANCO,String);
new String2[120];
format(String2,sizeof(String2),"¿Dueño?:%s",InfoCoche[Coche][Propietario]);
...
CMD:aplicar(playerid,params[]) {
new CocheID = GetPlayerVehicleID(playerid);
InfoCoche[CocheID][Comprado] = 1;
return 1;
}
