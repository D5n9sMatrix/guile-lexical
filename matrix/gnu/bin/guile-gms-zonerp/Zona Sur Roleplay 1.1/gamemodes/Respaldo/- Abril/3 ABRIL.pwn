...
CMD:esposar(playerid, params[])
{
		...
		if(GetPVarInt(playerid, "Injured") == 1) return SendClientMessage(playerid, COLOR_GREY, "No puedes hacer esto ahora.");
		new string[128],
		    Float:Pos[3],
			giveplayerid;
		if(sscanf(params, "d", giveplayerid)) return SendClientMessage(playerid, COLOR_WHITE, "USA: /esposar [playerid]");
		if(IsPlayerConnected(giveplayerid))
		{
		    new Nombree[MAX_PLAYER_NAME]; GetPlayerName(giveplayerid,Nombree,sizeof(Nombree));
			if (PDS(8.0, playerid, giveplayerid))
			{
				if(giveplayerid == playerid) return SendClientMessage(playerid, COLOR_GREY, "No puedes hacer esto contigo mismo!");
