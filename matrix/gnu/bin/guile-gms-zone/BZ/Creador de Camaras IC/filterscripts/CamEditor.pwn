...
public OnFilterScriptInit()
{
        ...
        print(" Traduccion Español por Raylan Givens!");
        print("--------------------------------------\n");
        return 1;
}
...
CMD:salircameditor(playerid,params[])
{
  if(IsCreating[playerid])
  {
    ...
    coordInfo[playerid][MoveSpeed]          = 1000;
    coordInfo[playerid][RotSpeed]           = 1000;
    SendClientMessage(playerid, -1, "Usted salió del editor de movimiento de la cámara.");
    }
  return 0;
}
    ...
    if ((newkeys & KEY_FIRE) && !(oldkeys & KEY_FIRE))
    {
        if(IsCreating[playerid] == true)
        {
            if(SettingFirstLoc[playerid] == true)
                        ...
                        {
                                        SendClientMessage(playerid, -1, "{8EFF8E}>{FFFFFF} Posición inicial {8EFF8E}re-set.");
                                        ShowPlayerDialog(playerid, DIALOG_MENU, DIALOG_STYLE_LIST,"{8EFF8E}Siguiente paso?","Ver Camara\nCambiar Inicio\nFin de Cambio\nCambiar Velocidad\nGuardar","Aceptar","Cancelar");
                                        IsReSettingStart[playerid]              = false;
                                        IsReSettingEnd[playerid]                = false;
                                        SettingFirstLoc[playerid]               = false;
                                        SettingLastLoc[playerid]                = false;
                                ...
                                {
                                    SendClientMessage(playerid, -1, "{8EFF8E}>{FFFFFF} Posición inicial {8EFF8E}set.");
                                    SendClientMessage(playerid, -1, "Ahora usa {F58282}~k~~PED_FIREWEAPON~ {FFFFFF}para guardar la posición de la cámara como el {F58282}Final {FFFFFF}posición.");
                        ...
                        else if(SettingLastLoc[playerid] == true)
                        {
                            const Float:fScale = 5.0;
                            new string[512];
                            format(string, sizeof(string), "Por favor introduce el deseado {F58282}movimiento{a9c4e4} tiempo en milisegundos\n\nVelocidad de movimiento actual: \t{F58282}%i milisegundos\n{a9c4e4}Velocidad de rotación actual: \t{F58282}%i milisegundos\n\nNota: {a9c4e4}1 segundo = 1000 milisegundos", coordInfo[playerid][MoveSpeed], coordInfo[playerid][RotSpeed]);
                            ShowPlayerDialog(playerid, DIALOG_MOVE_SPEED, DIALOG_STYLE_INPUT, "{8EFF8E}Velocidad de Movimiento", string,"Aceptar","Cancelar");
                            GetPlayerCameraPos(playerid, fPX, fPY, fPZ);
                            GetPlayerCameraFrontVector(playerid, fVX, fVY, fVZ);
                            object_x = fPX + floatmul(fVX, fScale);
                            object_y = fPY + floatmul(fVY, fScale);
                            ...
                            if(IsReSettingEnd[playerid] == true)
                                ...
                                {
                                    SendClientMessage(playerid, -1, "{8EFF8E}>{FFFFFF} Posición final {8EFF8E}set.");
                                    SettingLastLoc[playerid] = false;
                                }
                        }
                }
        ...
        switch(dialogid)
        {
                ...
                if(response)
                {
                        switch(listitem)
                        {
                                        ...
                                        case 1: //Change start
                                        {
                                            ...
                                            SendClientMessage(playerid, -1, "Use {F58282}~k~~PED_FIREWEAPON~ {FFFFFF}para establecer una nueva {F58282}posición {FFFFFF}inicial.");
                                        }
                                        case 2: //Change end
                                        {
                                            DestroyPlayerObject(playerid, noclipdata[playerid][flyobject]);
                                            ...
                                            SetPVarInt(playerid, "FlyMode", 1);
                                            noclipdata[playerid][cameramode] = CAMERA_MODE_FLY;
                                            SendClientMessage(playerid, -1, "Use {F58282}~k~~PED_FIREWEAPON~ {FFFFFF}para establecer una nueva posicion {F58282}final.");
                                        }
                                        case 3: //Change speed
                                        {
                                            new string[512];
                                        ...
                                        case 4: //Export
                                        {
                                            ShowPlayerDialog(playerid, DIALOG_EXPORTNAME, DIALOG_STYLE_INPUT, "{8EFF8E}Guardar movimiento","Escriba un nombre para la camara en movimiento","Aceptar","Cancelar");
                                        }
                        }
                        }
                        else
            ...
            case DIALOG_MOVE_SPEED:
            {
                if(response)
                {
                            ...
                            if(IsNumeric(inputtext))
                            {
                                ...
                                new string[512];
                                format(string, sizeof(string), "Por favor introduce el deseado {F58282}rotación{a9c4e4}tiempo en milisegundos\n\nVelocidad de rotación actual: \t{F58282}%i milisegundos\n{a9c4e4}Velocidad de rotación actual: \t{F58282}%i milisegundos\n\n\n{F58282}Nota: {a9c4e4}1 segundo = 1.000 milisegundos", coordInfo[playerid][MoveSpeed], coordInfo[playerid][RotSpeed]);
                                ShowPlayerDialog(playerid, DIALOG_ROT_SPEED, DIALOG_STYLE_INPUT, "{8EFF8E}Velocidad de rotación", string,"Aceptar","Cancelar");
                                IsReSettingStart[playerid] = false;
                                IsReSettingEnd[playerid]   = false;
                                        }
                            else
                            {
                                ...
                                {
                                    new string[512];
                                    format(string, sizeof(string), "Por favor introduce el deseado {F58282}movimiento{a9c4e4} tiempo en milisegundos\n\nVelocidad de movimiento actual: \t{F58282}%i milisegundos\n{a9c4e4}Velocidad de rotación actual: \t{F58282}%i milisegundos\n{FF0000}Es necesario introducir un valor\n\n{F58282}Nota: {a9c4e4}1 segundo = 1.000 milisegundos", coordInfo[playerid][MoveSpeed], coordInfo[playerid][RotSpeed]);
                                    ShowPlayerDialog(playerid, DIALOG_MOVE_SPEED, DIALOG_STYLE_INPUT, "{8EFF8E}Velocidad de Movimiento", string,"Aceptar","Cancelar");
                                }
                }
                else
            ...
            case DIALOG_ROT_SPEED:
            {
                if(response)
                {
                    if(strlen(inputtext))
                    {
                            ...
                            {
                                new string[512];
                                format(string, sizeof(string), "Por favor introduce el deseado {F58282}rotación{a9c4e4} tiempo en milisegundos\n\nVelocidad de movimiento actual: \t{F58282}%i milliseconds\n{a9c4e4}Current rotation speed: \t{F58282}%i milisegundos\n{FF0000}Numeros Solamente!\n\n{F58282}Nota: {a9c4e4}1 segundo = 1.000 milisegundos", coordInfo[playerid][MoveSpeed], coordInfo[playerid][RotSpeed]);
                                ShowPlayerDialog(playerid, DIALOG_ROT_SPEED, DIALOG_STYLE_INPUT, "{8EFF8E}Velocidad de rotación", string,"Aceptar","Cancelar");
                            }
                                }
                                else
            ...
            case DIALOG_EXPORTNAME:
            {
                ...
                if(strlen(inputtext))
                {
                                ...
                                {
                                    ShowPlayerDialog(playerid, DIALOG_EXPORTNAME, DIALOG_STYLE_INPUT, "{8EFF8E}Guardar movimiento","Escriba un nombre para el movimiento\n{FF0000}Es necesario introducir un texto","Aceptar","Cancelar");
                                }
                        }
                else
                {
...
public ExportMovement(playerid, inputtext[])
{
    ...
    new rotstring[512];
    new filename[50];
    format(filename, 128, "CamEditor_%s.txt", inputtext);
    ...
    format(myOutpString, sizeof(myOutpString), "Movimientos de la cámara guardan en la carpeta scriptfiles {F58282}%s {a9c4e4}\n\n¿Qué quieres hacer ahora?", filename);
    ShowPlayerDialog(playerid, DIALOG_CLOSE_NEW, DIALOG_STYLE_MSGBOX,"{8EFF8E}¿Y ahora qué?",myOutpString,"Crear nuevo","Salir");
}
...
stock FlyMode(playerid)
{
        ...
        SetPVarInt(playerid, "FlyMode", 1);
        noclipdata[playerid][cameramode] = CAMERA_MODE_FLY;
        SendClientMessage(playerid, -1, "Ha introducido el editor de movimiento de la cámara.");
        SendClientMessage(playerid, -1, "Puede usar /salircameditor para cerrarla.");
        SendClientMessage(playerid, -1, "con {F58282}~k~~GO_FORWARD~, ~k~~GO_BACK~, ~k~~GO_LEFT~ and ~k~~GO_RIGHT~{FFFFFF} puede mover la cámara.");
        SendClientMessage(playerid, -1, "Use {F58282}~k~~PED_FIREWEAPON~ {FFFFFF}para guardar la posición de la cámara{F58282} iniciar {FFFFFF}posición.");
        return 1;
}
